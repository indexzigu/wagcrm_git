#!/usr/bin/env bash
# 구 플랫폼(Vercel) 롤백 창구 승격 — main 에 쌓인 머지를 release 로 올려
# "승격 1회 = Vercel 빌드 1회"로 배칭한다.
#
# ⛔ **이 스크립트는 더 이상 프로덕션 배포 경로가 아니다(2026-08-13 자체호스팅 컷오버).**
#   종전 헤더 서술 "P6 배포 정본(2026-07-24)"은 SUPERSEDED — 운영 트래픽은 자체호스팅이
#   받고, 배포 판정 정본은 셀프호스트 배포 마커다(P6 「Deployment Verification」 ·
#   scripts/await-promotion.sh 기본 레인 selfhost). `release` 는 **롤백할 때만 수동으로**
#   올리는 창구이고, 전진하지 않으므로 "낡았다"가 결함이 아니다. 자동 승격은 꺼져 있다.
#   ⚠️ 즉 이 파일을 실행하는 것은 "내 커밋을 prod 에 올리는" 행위가 아니다 — 그 목적으로
#   왔다면 P6 를 먼저 읽어라(잘못 실행해도 프로덕션은 바뀌지 않지만, 바뀌었다고 오인한
#   보고가 나간다). 아래 배경·안전장치 서술은 그 롤백 창구 안에서 여전히 유효하다.
#
# 배경: PR 머지 = 즉시 빌드 시절, main 머지 50회/4일 × 빌드당 ~20 CPU분이
# Pro 포함 크레딧을 소진했다. main 은 빌드 없는 통합 브랜치로 두고,
# 배포하고 싶은 시점에 이 스크립트로 승격한다.
#
# 사용법:
