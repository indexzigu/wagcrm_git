...
/**
 * 항목의 **뒤 상태 서술 구역** — 아래 두 함수(`trailingRegion` · `trailingStatusPhrase`)가
 * 공유하는 배경이다. 왜 이 구역이 필요한가:
 *
 * 🪤 **보드 줄 형식이 둘로 갈렸다(실측 2026-09-09).** 옛 형식은 `<상태> — <제목>` 이라 위
 * `statusPhrase` 가 상태를 정확히 집었는데, 새로 올라오는 항목은 하네스 세션명 규약
 * (`<상태이모지> #<번호> [<계보슬러그>] <제목>`)을 그대로 헤더로 쓰면서 **상태 서술을
 * ` — ` 뒤에** 적는다. 그래서 대기 주장이 판정 구역 **밖**으로 나갔고, 같은 문구·같은
 * PR·같은 배포 사실인데 줄 형식만 다르면 옛 형식은 빨강, 새 형식은 초록이 됐다 — 머지·배포가
 * 끝난 항목이 "아직 대기"로 남아도 이 점검기가 "드리프트 없음"을 보고했다(= 2026-07-29
 * 사고의 새 형식판 재발).
 *
 * ⛔ **줄 전체로 넓혀 메우지 말 것** — 본문 서술을 주장으로 오독하는 2026-08-05 거짓 경보가
 * 그대로 되돌아온다(위 `statusPhrase` 주석). 구역은 보드 규약이 실제로 쓰는 **필드 구분자
 * ` · `**(AGENTS.md 보드 아키텍처의 `상태 · 다음 게이트 · 상세 링크`)로 끊어 **첫 필드만** 본다.
 *
 * 🪤 **구분자를 구역에 포함하지 말 것.** ` · ` 를 함께 넣으면 `⏳ CI 대기 → 오너 머지 ·` 가
...
/**
 * 뒤 구역의 **원문** — 헤더 안의 첫 ` — ` 뒤부터 헤더 끝까지. 필드로 자르는 것은 아래
 * `trailingStatusPhrase` 가 한다.
 *
 * 🪤 **구역은 반드시 헤더 안이다(실측 2026-09-09).** 처음 구현은 줄 전체에서 첫 ` — ` 를 찾았는데,
 * 실보드 202줄에 돌려 보니 **본문에만 ` — ` 가 있는 서술 줄**이 걸렸다(`- **패스한 것**: … — …
 * 「오너 머지 대기」로 남아 있으나 실제로는 MERGED …`). 그 본문은 다른 항목의 경과를 **설명**한
 * 것인데 주장으로 읽혀 낡은 마커 오탐이 났다 — 2026-08-05 거짓 경보와 같은 계열이다. `headerEndOf`
 * 로 잘라 두면 본문의 ` — ` 는 애초에 후보가 아니다.
 *
 * 🪤 **강조 표식은 경계가 아니라 잡음이다(리뷰 실측 2026-09-10).** 첫 `**` 에서 자르면 실보드의
 * 지배적 형태인 `— 머지·**prod 반영 완료** · 종결` 에서 구역이 `머지·` 로만 남아 **강조된 상태
 * 문구를 못 읽고**(`— 머지·**오너 머지 대기**` 가 통과했다 — 고치려던 침묵 그대로), 반대로 표식을
 * 남겨 두면 `머지 **완료**` 가 `MERGE_DONE` 에 안 걸려 **정상 배포 대기가 낡은 마커로 뒤집힌다**.
 * 양쪽 다 재현 확인했다 — 그래서 경계로도 쓰지 않고 남기지도 않는다. 걷어낸다.
 *
 * ⛔ **경계를 문장 부호(`. `)로 잡으려던 안은 기각됐다(리뷰 실측 2026-09-10).** 그렇게 끊으면
 ...
 * 하나로 족하다. 필드 안에 경과 서술이 섞이는 경우를 **머지 축에서 걸러내려던 시도는 기각됐다**
 * (아래 `claimsAwaitingMergeIn` 주석) — 배포 축만 `PROMOTION_WAIT` 의 어미 판별로 과거형 서술을
 * 뺀다(그쪽은 낱말 둘이 붙어야 매치해 실효가 있다).
 */
function trailingRegion(line) {
  ...
  // 강조 표식은 경계가 아니라 **잡음**이다 — 걷어내고 읽는다.
  return line.slice(start, Math.min(headerEnd, start + REGION_CAP)).replace(/\*\*/g, "");
}

/**
 * 주장을 읽는 뒤 구역 = 필드 목록의 **첫 필드**(` · ` 앞). 보드 규약에서 그 자리가 상태 필드다.
 *
 * 🪤 **필드를 넘어 읽지 말 것.** 머지 대기 판정은 `머지` 와 대기 신호(`대기`·⏳·🔴)가 같은 구역에
 * 있으면 발화하므로, 필드를 넘으면 **필드 1의 `머지` + 게이트 필드의 `대기`** 가 엮여 끝난 항목이
 * 낡은 마커로 뜬다(픽스처: `머지(sha) 후 prod 반영 · 다음 게이트 = 오너 육안 대기`).
 * `승격 대기` 관용구만 예외로 넓히려던 안도 리뷰 실측에서 기각됐다 — 뒤쪽 필드는 서술이라
 ...
 * **둘 다** 요구한다.
 *
 * 🪤 **PR 참조는 태그가 아니다(리뷰 실측 2026-09-09).** 마크다운 링크(`[PR #5](…)`)는 뒤에 `(` 가
 * 붙어 걸러지지만, 실보드에는 **링크 아닌 맨 `[PR #268]`** 로 적힌 줄이 있다(취소선 안에 들어 있다).
 * 그걸 태그로 세면 그 줄이 새 형식으로 재분류돼 **취소선 본문을 상태로 읽고** 거짓 낡은 마커가 난다
 * (재현 확인 — 종료코드를 흔드는 쪽이라 설계 원칙 3 정면). 그래서 태그 안의 `#` 를 배제한다
 * (계보 슬러그에는 `#` 가 없다 — `[PR최소화]` 처럼 PR 로 시작하는 슬러그는 그대로 통과한다).
 *
 * ⚠️ **잔여 사각 ①:** 계보 태그 없이 올린 새 형식 줄은 옛 형식으로 읽혀 뒤 구역을 보지 않는다.
 * 실측 2026-09-10(실보드 항목 216줄 기준): 앞 구역에 `#번호`와 ` — ` 를 함께 가진 줄 19건 중
 * **11건이 인식되고 8건은 태그가 없어 옛 형식으로 읽힌다.** 보드는 다세션이 계속 쓰는 파일이라
 * 이 수는 곧 달라지지만, 덮이지 않는 쪽의 처방은 코드가 아니라 **세션명 규약대로 태그를 적는 것**이다. 이 방향의 오류는
 * 침묵(미탐)이고 반대 방향은 거짓 경보라 설계 원칙 3 에 따라 이쪽을 택했다 — 태그는 하네스 세션명
 * 규약이 이미 요구하므로 규약을 지키면 덮인다.
 * ⚠️ **잔여 사각 ②:** 옛 형식이면서 앞 구역에 번호와 태그가 **둘 다** 우연히 든 줄
 * (`✅ #5 [T-020] 머지·prod 반영 완료 — <제목>`)은 새 형식으로 오분류된다. 두 형식이 이 지점에서
 * 구조적으로 구별되지 않는다 — 지금 보드에는 그 형태가 없고, 있으면 태그를 빼거나 옛 형식으로 통일
 * 하는 것이 처방이다(코드로는 못 가른다).
 */
...
/**
 * 주장을 읽는 **상태 구역** — 형식에 따라 앞/뒤가 갈린다. **모든 축이 이 구역 하나를 읽는다**
 * (축마다 폭을 달리 두려던 안은 리뷰 실측에서 기각됐다 — 위 `trailingStatusPhrase` 주석).
 * 형식 분기는 이 함수에만 있다.
 *
 * ⛔ **머지·배포 주장 판독에서 `statusPhrase` 를 직접 쓰지 말 것** — 새 형식에서 제목을 상태로
 * 읽는다.
 ...
 * ℹ️ 종전에 여기 적혀 있던 「제목에 게이트 낱말이 들어가면 살아 있는 게이트로 읽힌다」는 **해소됐다**
 * (T-145) — 게이트 탐색 범위는 `gateSearchText` 가 형식을 인지해 좁힌다. 종결 표식만 앞 구역에서
 * 읽는 이 비대칭이 그대로 남는 것이 의도다.
 */
 ...
 * 를 자백으로 읽어 **배포까지 끝난 항목이 드리프트로 뒤집혔다**. 「완료가 앞서면 뒤는 서술」로
 * 가르는 위치 규칙은 이 축에 쓸 수 없다 — 옛 형식은 `머지 완료 · 승격(배포) 대기` 처럼 **완료
 * 뒤에 대기를 적는 것이 정상 표기**라(2026-08-08 실사고의 그 문구) 정상 항목이 통째로 침묵한다.
 * (같은 이유로 머지 축에서도 그 규칙은 기각됐다 — 아래 `claimsAwaitingMergeIn` 주석.)
 * 그래서 여기서는 어미로 가른다 — 낱말 둘이 붙어야 매치하는 이 관용구에서만 실효가 있다.
...
/**
 * 고전 어순의 대기 관용구.
 * ⚠️ 여기 `(?!였|이었|했)` 같은 과거형 가드를 넣어도 **무력하다** — 아래 `WAIT_SIGNAL` 의 맨
 * `대기` 가 `대기였다` 를 다시 잡는다(변이로 확인: 가드를 지워도 계약 125건 전부 초록).
 * 과거형 판별이 실효를 갖는 곳은 낱말 둘이 붙어야 매치하는 `PROMOTION_WAIT` 쪽이다.
 */
...
/**
 * 줄 하나의 머지 대기 주장 — 판정은 base 그대로이고, **읽는 구역만** 형식을 인지한다.
 *
 * ⛔ **여기에 「완료가 앞서면 뒤엣것은 서술」 같은 위치 규칙을 얹지 말 것(리뷰 실측으로 기각,
 * 2026-09-10).** 새 형식 상태 필드에 경과 서술이 붙는 경우를 잡으려고 넣었는데, 완료 신호를
 * 무엇으로 보든 **실제 대기 주장이 함께 침묵했다**:
 *   · `머지·배포 대기. 오너 머지 대기` — `머지·` 를 완료로 오인(그 패턴은 나열 구분자다)
 *   · `1차 머지·배포 완료 / 2차 오너 머지 대기` — 회차를 나란히 적는 관행. 규칙을 새 형식에만
 *     걸면 **티켓이 문제 삼은 형식 비대칭이 반대로 되살아난다**(옛 형식 빨강 / 새 형식 초록).
 *   · `데모 첫 배포 완료 후 오너 머지 대기` — 완료 어휘를 배포까지 넓히면 데모 레인이 prod
 *     완료로 읽힌다(이 파일의 「데모 레인은 다른 축」 규칙 정면 위반).
 *
 * ℹ️ 그 대가로 **상태 필드에 경과 서술을 적은 줄은 대기 주장으로 잡힌다**
 * (`머지·prod 반영 완료. 본문: … 오너 머지 대기 로 남아 있었다는 설명`). 그건 받아들인다 —
 * 규약상 그 자리는 상태 필드이고, 이 도구의 출력은 **고칠 보드 줄을 지목하는 것**이 일이다.
 * 침묵과 달리 빨강은 눈에 띈다.
 * ⚠️ 다만 그 처방이 **보드를 고치는 것**이라는 점을 가볍게 보지 말 것 — 보드는 git 미추적
 * 다세션 공유 파일이라 쓰기 자체가 P0 위험이다(AGENTS.md 보드 아키텍처: 앵커 치환·레포 밖
 * 백업·타 세션 줄 무손상 확인). 서술은 상태 필드가 아니라 본문 필드에 적는 것이 더 싸다.
 */
function claimsAwaitingMergeIn(line) {
  return claimsAwaitingMerge(statusRegion(line));
}
...
function printReport(boardPath, results, deploy) {
  ...
  if (legacy.length) {
    console.log(
      // T-104: **번호를 나열하지 않는다.** 이 부류의 결론은 「할 일 없음」이라 매 실행마다
      // 26건 규모의 번호 벽을 찍으면, 정작 봐야 할 줄(드리프트·대기)을 밀어낸다 —
      // 오너가 매일 같은 항목을 다시 들여다보게 되던 것이 이 티켓의 증상이다.
      // ⛔ **분류를 바꾼 게 아니다** — 판정도 종료코드도 그대로이고 출력 분량만 줄인다.
      //    목록이 필요하면 아래 한 줄로 언제든 뽑을 수 있다(보드가 이미 레포를 밝힌다).
