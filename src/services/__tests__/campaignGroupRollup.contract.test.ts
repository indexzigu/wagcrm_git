 ...
 * 다른 오동작이 된다). 대신 **그 전제 자체를 여기서 고정한다.**
 * ℹ️ 갱신 타입 쪽 구멍은 닫혔다 — `CampaignGroupRollupUpdate` 의 롤업 두 필드가 `null` 을
 * 받지 않는다. 남은 것은 **컬럼이 여전히 nullable** 이라는 것이고(생성 직후 한 순간 비어
 * 있어야 해서 그렇다), 그 구간을 지키는 것이 아래 계약이다.
 */
