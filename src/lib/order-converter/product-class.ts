 ...
 *
 * ⚠️ **도입 시점(2026-09-02) 저장 데이터에는 NFD 가 없었다** — 관측된 `productClass` 값이
 * 전부 NFC 였고, 옛 판정(`=== 리터럴`)과 이 판정이 갈리는 행도 없었다. 즉 이 정규화는
 * **그때 무엇을 고친 것이 아니라** 형태가 바뀌었을 때 조용히 갈리지 않게 하는 예방이다.
 * ⛔ 이 문장을 「정규화가 버그를 고쳤다」로 바꿔 적지 말 것.
 *
 * **재현 방법**(레포 안에 계측기를 두지 않는 이유: 1회성 판정이고 프로덕션 DB 접근이 필요하다):
 * `NaverOrderSnapshot.orders` 블롭을 펼쳐 `productClass` 를 값별로 모은 뒤, 각 값에서
 * ①코드포인트 길이가 글자 수와 같은지(NFD 면 2~3배로 늘어난다) ②`normalize(v, NFC) = v`
 * 인지를 본다. 「갈리는 행 0」은 `(v = 리터럴)` 과 `(normalize(v, NFC) = 리터럴)` 이 서로
 * 다른 행을 세어 확인한다. 🪤 그 「0」에는 **양성 프로브를 짝지어라** — 인위적 NFD 값을
 * 하나 넣어 갈림이 실제로 세어지는지 봐야 0 이 스캔 고장이 아님이 확정된다.
 *
