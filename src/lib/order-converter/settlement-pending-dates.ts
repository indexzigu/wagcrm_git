 ...
 * 삼키지 않고 고지한다).
 *
 * **배분은 「재확인 몫 예약 + 나머지는 오래된 순」이다.** 오래된 쪽이 상한(§`MAX_AGE`)에
 * 먼저 닿아 기회가 적으므로 남은 자리는 오래된 날짜가 가져간다.
 * ⛔ **그렇다고 `allDates.slice(0, MAX)` 로 되돌리지 말 것** — 종전 주석은 "최근 날짜는
 * 어차피 다음 회차에 다시 들어온다"고 적었으나 **그것이 거짓임이 확인됐다**: 옛 날짜가
 * 상한을 채운 채 하루에 하나씩만 빠지면 포화가 재확인 창보다 오래 가고, 그 사이 오늘
 * 주문일이 창 밖으로 나가 영영 계획에 못 오른다. 예약 배분은 그 반증에서 나온 것이지
 * 불필요한 복잡도가 아니다.
 * ⚠️ 예약이 성립하려면 `SETTLEMENT_ORDER_DATE_RECHECK_DAYS <= SETTLEMENT_MAX_DATES_PER_RUN`
 * 이어야 한다(아니면 이번엔 오래된 날짜가 굶고 상한이 무력해진다) — 계약 테스트가 고정한다.
 */
