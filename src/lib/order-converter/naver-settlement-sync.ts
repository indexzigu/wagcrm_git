 ...
 * ⚠️ **단 확인 기간 안에서만이다**(post-close-check-window.ts — 전 주문 종결 +10일 · 판매 종료
 * +15일 안전선). 그 뒤에 락이 걸리는 캠페인은 확정 계산이 없고 기간 안 마지막 값이 최종이다 —
 ...
 * 캠페인은 이 전제를 영영 못 채우고 판매 종료 +15일 안전선까지 매일 재조회된다 — 알고 택한
 ...
 * 종결이 끝내 안 보이는 캠페인은 판매 종료 +15일 안전선에서 멈춘다. 종전 90일 창을 대체했다
