...
/**
 * 전수 스캔 항목의 시간 예산. **기본 5000ms 는 단위 테스트용 예산이라 900여 파일을 AST 로 훑는
 * 항목에 애초에 맞지 않는다** — 이 레포는 그런 항목에 30초를 명시한다(선례:
 * `src/lib/__tests__/resident-number-exposure.contract.test.ts` 의 `REPO_SCAN_TIMEOUT_MS`).
 * 그 항목이 없어 이 파일은 병렬 부하에서 반복 실패했다(유휴 소요 1.4초 — 코드가 느린 것이 아니다).
