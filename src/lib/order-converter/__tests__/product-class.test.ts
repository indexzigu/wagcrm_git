...
describe('isSupplementProduct — 추가구성상품 판정', () => {
  it('NFC(조합형) 값을 참으로 본다 — 도입 시점 네이버가 보내던 형태다', () => {
    // ⚠️ 상대가 정하는 형태라 확인 시점의 사실이다 — 다시 확인하는 방법은 SSOT 헤더.
  ...
  it('메인 품목 분류는 거짓이다', () => {
    // 메인 품목 쪽 분류값(네이버 어휘). 정규화가 이것까지 삼키면 안 된다.
    // ⚠️ 상대가 정하는 값이라 레포 안에 근거가 없다 — 다시 확인하는 방법은 SSOT 헤더.
  ...
  it('NFC 입력에서는 종전 리터럴 비교와 결과가 같다 — 이 리팩터가 수치를 바꾸지 않는 근거', () => {
    ...
    //
    // 도입 시점 대조에서도 두 판정이 갈리는 자리는 없었다(확인 방법은 SSOT 헤더).
