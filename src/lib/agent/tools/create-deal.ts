...
export const createDealTool: AgentTool<CreateDealInput, CreateDealData> = {
  ...
  description:
    ...
    // ⛔ "이름만 주면 알아서 붙는다"로 적지 말 것 — 상호가 같은 거래처가 여럿일 때 어느
    //    거래처에 딜이 붙었는지 아무도 고르지 않은 채 정해진다. search_partners(2026-09-10
    //    추가)는 id 를 **보여주는** 조회이고, 고르는 일은 사람 몫이다.
    "거래처는 반드시 하나로 지정해야 합니다 — 이번에 새로 만들 거래처면 partner에 그 정보를 " +
    "담습니다. 이미 등록된 거래처에 붙이려면 그 거래처의 내부 id(partnerId)가 필요한데, " +
    "search_partners로 상호를 검색해 후보를 사용자에게 보여주고 사용자가 고른 거래처의 id를 " +
    "partnerId에 넣으십시오. 같은 상호가 여러 건이면 임의로 고르지 말고 사용자에게 물어보고, " +
    "id를 지어내지 마십시오 (둘 다 주면 거부됩니다). " +
