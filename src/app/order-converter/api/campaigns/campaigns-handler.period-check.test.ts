 ...
 *
 * 지켜야 하는 불변식: **유휴 만기가 이번 회차의 스토어 조회를 부른 경우에만**, 그 회차에
 * **유휴 구간 캠페인 전원**의 `periodCheckedAt` 을 함께 찍는다(두 조건이 짝이다).
 *
 ...
 *
 * 🪤 거꾸로 **조회가 났다는 이유만으로 찍어도 안 된다** — 임박·기간미확정 캠페인이 하나라도
 * 있으면 조회는 매 GET 나므로, 유휴 캠페인 전원이 매 GET 쓰기를 받는다. 그 회차는 추가 호출을
