...
async function handler(request: Request): Promise<Response> {
   ...
   *
   * 상한은 **건수가 아니라 직렬화 바이트로** 건다. 항목이 길어질 때 건수만 제한하면
   * 이 필드 하나가 `SystemTaskLog.details` 봉투(4,000자)를 통째로 먹기 때문이다.
   * ⛔ 종전 근거 「넘으면 객체 전체가 `{truncated, preview}` 로 대체돼 앞선 집계 숫자까지
   * 사라진다」는 **SUPERSEDED**(T-084) — 저장부가 이제 문자열을 자르지 않고 **가장 큰
   * 배열부터 줄이며 요약 필드는 보존**한다(`capDetailsForLog`). 그래도 이 예산을 유지하는
   * 이유는 바뀌지 않았다: 여기서 미리 줄여야 **어느 항목이 덜어졌는지를 이 잡이 스스로
   * 고지**할 수 있고(`capped`), 저장부에 맡기면 그 판단이 잡 밖으로 나간다.
