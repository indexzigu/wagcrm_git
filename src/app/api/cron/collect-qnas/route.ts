...
import { QNA_DEFAULT_LOOKBACK_DAYS, runQnaSync } from "@/lib/order-converter/naver-qna-sync";
...
/**
 * 상품 문의(VOC) 주간 수집(매주 월) — 상품문의 + 고객문의. SSOT: REVIEW_QNA_COLLECTION_PLAN.md.
 * 쿼리 파라미터(수동 백필용): ?lookbackDays=31 (기본 QNA_DEFAULT_LOOKBACK_DAYS = 8 — 주 1회 + 경계 겹침).
 */
async function handler(request: Request) {
  ...
  try {
    ...
    const lookbackDays = Math.min(Math.max(Number(url.searchParams.get("lookbackDays")) || QNA_DEFAULT_LOOKBACK_DAYS, 1), 31);
