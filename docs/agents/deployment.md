...
- **`deploy.sh` 를 고치는 커밋의 첫 배포는 옛 `deploy.sh` 로 돈다 — 새 로직은 그 다음
  배포부터다 (2026-08-29 등재):** `deploy.sh` 는 실행 도중(현재 `git reset --hard
  "origin/$TRACK_BRANCH"`) **자기 자신을 포함한 체크아웃 전체를 갱신한다.** git 은 파일을
  제자리에서 고치지 않고 **새 inode 로 갈아 끼우므로**, 이미 그 파일을 열고 있는 bash 는
  **옛 내용을 끝까지 읽는다.** 그래서 그 배포는 처음부터 끝까지 옛 절차로 완주하고,
  갓 머지한 배포 로직은 **한 번도 실행되지 않은 채** 성공으로 보고된다.
  - **실측(2026-08-29, 임시 레포 프로브):** 실행 중 스크립트가 `git reset --hard` 로
    교체되자 `ls -i` 의 inode 가 바뀌었고(교체는 성립했고), 그럼에도 **교체 이후 줄까지
    구 본문이 전부 실행**된 뒤 `exit 0` 이었다.
  - ⚠️ **위 「전환기 폴백」과 같은 계열이지 같은 사건은 아니다.** 저쪽은 *산출물*이 옛
    형태라 첫 배포가 폴백으로 뜨는 것이고, 이쪽은 *절차 자체*가 옛 것이라 새 안전장치·새
    마커·새 검증이 그 회차에 통째로 없다. 둘이 겹치는 배포에서는 「경고 한 줄이 정상」
    이라는 저쪽 설명이 이쪽의 부재까지 덮어 주는 것처럼 읽히니 주의할 것.
  - **그래서 배포 로직 변경은 2회 돌린다.** 1회차는 착지시키는 회차(옛 절차)이고
    2회차가 새 절차의 **첫 실전**이다. 새 절차의 실동작 확인(마커·릴리스 트리·안전장치
    ⑦⑧)은 **2회차에** 한다 — 1회차 로그에서 새 문구를 찾다가 "안 실렸다"고 오판하지 말
    것. 1회차 로그에 새 문구가 없는 것이 **정상**이다.
  - 🪤 **`cat >`·`sed -i` 같은 제자리 덮어쓰기였다면 증상이 전혀 달랐다** — bash 는 파일
    오프셋으로 읽으므로, 짧아진 파일에서는 **남은 줄을 통째로 건너뛰고 `exit 0`** 이고
    길어진 파일에서는 **토큰 중간부터 이어 읽어 `command not found`** 가 된다(같은 날
    프로브로 둘 다 재현). 즉 "조용한 중도 종료"는 실재하는 실패 모드이지만 **지금의 git
    경로에서는 나오지 않는다.** ⛔ 배포 중 자기 갱신을 git 이 아닌 제자리 쓰기로 바꾸지
    말 것 — 지금 무해한 성질이 그 순간 무증상 부분 실행으로 바뀐다.
  - ⚠️ **2026-08-29 에 배포를 두 번 돌렸고 1회차가 중단됐다는 관찰이 있으나, 그 중단의
    원인은 이 성질로 확정되지 않았다**(오너 터미널 출력 미확인 + 위 실측대로 git 경로는
    완주한다). 여기 등재하는 것은 **구조적 성질**이고 그날의 중단은 정황이다.
...
- **배포완료 자동통지 — `await-promotion.sh` (2026-07-24 도입 / 2026-08-19 레인 이관):**
  ...
  **셀프호스트 배포 마커의 조상 여부**로 판정한다(위 Deployment Verification ②와 같은
  ...
  - **추가 종료코드 `5` = 판정 불가**(마커 부재·비-SHA·git 이 모르는 SHA·**마커와 공통
    조상이 없는 커밋** — 저장소 이관으로 이력이 갈린 구 커밋이 그 경우다, T-104. 이쪽은
    기본 레인 한정이고 `--lane vercel` 은 여전히 그 경우에 3 을 낸다). ⛔ **5 를 3 으로
