...
- **Main Push Guard & Deploy CI Gate — 브랜치 보호의 대체 방어 2문 (T-069, 2026-08-27):**
  ...
  - **메뉴바 `preflightRunner` 행의 경보 의미가 뒤집혔다:** 종전엔 「required 라서 러너
    ...
    직전에 잡으므로, 이 행은 "배포가 막히기 전에 미리 아는" 조기 신호로 역할이 바뀌었다.
    ⛔ **짝이던 `actionsQuota` 행은 2026-08-29 오너 결정으로 제거됐다** — 종전 서술은 두
    행을 함께 묶었지만, 그쪽이 지키던 「한도 소진」은 08-28 공개 레포 이전으로 **한도
    자체가 사라져** 성립하지 않는다(공개 레포 실행은 계량은 되되 청구가 붙지 않는다).
    남은 것은 결과 없는 숫자가 옛 한도를 넘긴 채 굳은 **상시 노랑**이었고, 늘 켜진 경고는
    곧 무시당해 진짜 빨강까지 삼킨다. 되살릴 사유는 「레포를 다시 비공개로 돌린다」
    하나이고, 그때의 판정 기준·묘비는 `infra/selfhost/status.sh` 의 같은 자리에 있다.
...
- **배포는 서빙 중인 산출물을 덮어쓰지 않는다 — 빌드 트리와 서빙 트리를 가른다
  (실사고 2026-08-29):** `deploy.sh` 는 앱을 **내리지 않은 채** `npm run build` 를
  돌리는데, Next 는 `cleanDistDir: true` 라 빌드 시작 시 `.next` 를 통째로 비운다.
  종전에는 앱이 바로 그 `.next/standalone/server.js` 를 서빙했으므로, 빌드가 도는
  내내 살아 있는 구 프로세스가 지연 로딩하려던 청크·클라이언트 참조 매니페스트가
  사라져 **그 사이 들어온 요청이 전부 죽었다.**
  - **실측:** 빌드 시작~완료 **63초** 구간에서 `InvariantError`(client reference
    manifest 부재) 6건 · `ChunkLoadError` 2건 · `500.html` 부재 6건. `kickstart` 로
    새 프로세스가 뜨면서 정상화됐다(로그 mtime 기준 — 로그에 타임스탬프가 없어
    시각 대조는 `app.err.log`·`app.out.log`·`.next` 산출물의 mtime 으로 했다).
  - 🪤 **"Next.js 버그"로 읽지 말 것.** 메시지의 `This is a bug in Next.js` 는 내부
    불변식이 깨지면 무조건 붙는 정형 문구다. 에러가 "없다"고 말한 청크는 **빌드 완료 후
    실재했다** — 누락이 아니라 그 순간의 부재다.
  - 🪤 **"구 산출물이 남아 새 빌드와 섞였다"도 아니다.** `.next/server` 전체가 그
    빌드의 산출물이었다(옛 날짜 0건 — `cleanDistDir` 와 일치). ⛔ **처방으로
    `rm -rf .next` 를 넣지 말 것 — 역효과다:** 요청이 실패하는 구간이 더 길어지고,
    Turbopack 빌드 캐시까지 날아가 빌드 자체가 느려진다.
  - 🪤 **`ChunkLoadError` 의 스택을 범인으로 읽지 말 것 — 2차 실패다.** 그 청크는
    Sentry 계측(`instrumentation.onRequestError`)이라, 1차 에러를 **보고하려다** 자기
    청크를 못 읽은 것이다. 스택만 보면 계측이 원인처럼 보인다.
  - **구조:** 완성된 산출물을 `<체크아웃>/.live/releases/<sha>` 로 **옮기고**(같은
    파일시스템 rename 이라 즉시) `.live/current` 심링크만 바꿔 끼운다. 구 프로세스가
    보던 릴리스는 교체 후에도 남으므로 재기동 전까지 무사하다. `run-app.sh` 는
    `.live/current/server.js` 를 실행한다. 노출은 63초 → 재기동 순간으로 줄어든다
    (완전 0 은 아니다 — 그건 blue-green 이 필요하고 별개 사안이다).
  - ⚠️ **릴리스 폴더를 재사용하지 않는다.** `FORCE=1` 재배포는 같은 SHA 로 다시 도는데
    그 폴더가 **지금 서빙 중인 릴리스**다. 충돌하면 접미사를 붙여 새 폴더를 만든다 —
    선제 삭제를 넣으면 이 장치가 막으려는 사고를 스스로 일으킨다.
  - 🪤 **심링크 교체의 `mv` 가 대상을 따라가지 않게 하는 옵션을 빼면 조용히 실패한다.**
    대상 `current` 가 이미 디렉터리를 가리키는 심링크면 `mv` 는 링크를 **따라가** tmp 를
    그 디렉터리 **안으로** 옮기고 `exit 0` 을 낸다 — 링크는 옛 릴리스를 계속 가리키는데
    배포는 성공으로 보인다. 첫 배포만 우연히 맞고(대상 부재) 그 뒤로는 영영 갱신되지
    않는다. 소스 리뷰로는 안 잡혔고 **실행해 보고서야** 드러났다(4회 교체 후 current 가
    1회차를 가리킴).
  - 🪤 **그 옵션의 이름이 구현마다 다르다 — BSD/macOS `-h`, GNU/Linux `-T`.** 프로덕션은
    macOS 뿐이라 `-h` 로 고정했는데, 행위 계약이 그 블록을 **원본에서 발췌해 CI(Linux)에서
    실행**하므로 `mv: invalid option -- 'h'` 로 넘어졌다. `mv --version`(GNU 만 지원) 으로
    갈라 쓴다. ⛔ 한쪽으로 "단순화" 하지 말 것 — 그러면 행위 계약이 CI 에서 영영 못 돌고,
    앵커 계약만 남아 위 함정을 다시 놓친다.
    🪤 **이 부류를 로컬에서 미리 잡으려면 GNU 를 흉내내는 `mv` shim 을 PATH 앞에 두고
    돌린다.** 단 shim 은 **양성·음성 프로브로 먼저 검증할 것** — 초판이 `-h` 단독만 거부하고
    묶인 `-fh` 는 통과시켜, 변이를 주입해도 초록이 나왔다(즉 「shim 이 안 붙었다」는 정반대
    결론으로 갈 뻔했다). 프로브 없는 하네스는 고장이 통과로 보인다.
  - ⚠️ **`.live` 는 체크아웃 안에 둔다(밖이 아니라).** 프리뷰 레인의 서빙 트리에는
    **프로덕션 사본 DB 로 프리렌더된 페이지**가 들어가므로 `preview.sh down` 이 반드시
    함께 지워야 한다(오너 확정 2026-08-13 「잔여 사본 0」). 체크아웃 안이면 기존
    심링크·물리경로·`.git` 3중 가드가 그대로 덮는다 — 밖에 두면 가드 밖 경로에 대한
    재귀 삭제가 하나 더 생긴다. `.gitignore` 의 `/.live/` 와 재귀 삭제 허용 목록
    (`preview-control.test.ts`)이 짝이다.
  - ⚠️ **폴백은 전환기 전용이다 — 「안전망」으로 남겨두려고 `mv` 를 `cp` 로 바꾸지 말 것.**
    `run-app.sh` 는 `.live/current` 가 없으면 경고를 남기고 빌드 트리로 폴백한다(plist 가
    `KeepAlive` 라 여기서 `exit` 하면 크래시루프 = 서비스 전면 정지이기 때문이다).
    그런데 배포가 산출물을 **`mv`** 로 옮기므로 릴리스 방식으로 한 번이라도 배포되면
    `.next/standalone` 자체가 사라진다 ⇒ 폴백 조건이 성립하지 않고, `.live` 까지 없으면
    크래시루프(**시끄러운 고장**)가 된다. 즉 「폴백으로 조용히 서빙」이 가능한 구간은
    **이 변경이 착지한 직후 한 번**뿐이다(그 배포는 구 `deploy.sh` 가 돌던 중에 새
    `run-app.sh` 를 받는다 — 그때 `app.err.log` 에 `[run-app] ⚠️ 경고` 한 줄이 남는 것이
    **정상**이고 다음 배포가 해소한다). `cp` 로 바꾸면 빌드 트리가 늘 남아 폴백이 **영구히
    성립 가능**해지고, 이 보장이 통째로 무증상 열화로 바뀐다.
    짝 탐지: 안전장치 ⑧ 이 **헬스체크를 통과한 뒤** 앱 프로세스의 **cwd** 를 읽어 릴리스
    트리를 서빙 중인지 확인하고, 아니면 배포를 실패시킨다(마커 미갱신 → 다음 실행이 재시도).
    - 🪤 **`ps` 로 판정하지 말 것 — Next 가 프로세스 이름을 갈아치운다.** 실측(2026-08-29):
      `ps -o command= -p <pid>` 는 `next-server (v16.2.4)` 만 준다. 실행 경로가 거기 없으므로
      경로 대조는 **정상 배포에서도 항상 실패**한다. 초판이 그렇게 짜여 있었고, 첫 배포로
      릴리스가 만들어진 **다음 배포부터 전부 중단**될 상태였다(운영 중 프로브로 잡았다).
      standalone 서버가 기동 직후 `process.chdir(__dirname)` 을 하므로 **cwd 가 곧 서빙
      중인 릴리스**다 — `lsof -a -p <pid> -d cwd` 로 읽는다.
    - ⚠️ 확인을 **헬스체크 뒤**에 두는 것도 계약이다: 200 을 받았다는 것은 그 chdir 이 이미
      끝났다는 뜻이라, PID 교체 직후에 읽을 때 생기는 경합이 없다.
    - ⚠️ Node 는 심링크를 실경로로 푸므로 cwd 는 `.live/releases/<id>` 다 — `current`
      문자열과 직접 비교하지 말고 실경로로 푼 값과 댄다.
    - 🪤 **`RUNNING_CWD="$(… | head -1)"` 에 `|| true` 를 빠뜨리지 말 것** — `set -o pipefail`
      아래에서 `lsof` 실패가 **아무 메시지 없이** 그 줄에서 스크립트를 죽여, 바로 아래
      「판정 불능」 안내가 통째로 무력화된다(없는 PID 프로브로 실측).
  - ⚠️ **정리(prune) 실패는 배포 실패가 아니다.** 정리는 마커를 쓴 **뒤**에 돌므로 그
    시점엔 배포가 이미 성공했다. `set -e` 로 죽으면 성공한 배포가 실패로 보고돼 헛된
    재배포·경보를 부른다 — 실패는 말하되 종료코드를 바꾸지 않는다.
  - ✅ **롤백이 빌드 없이 된다 — 단, 마커를 함께 되돌려야 한다.** 절차는
    ①`.live/current` 심링크를 이전 릴리스로 교체(`ln -sfn` → `mv -fh`) ②`launchctl
    kickstart` ③**배포 마커(`deployed.sha`)를 그 릴리스의 SHA 로 갱신**이다.
    ⛔ ③ 을 빠뜨리지 말 것 — 마커는 위 「Deployment Verification」 ②가 **"지금 서빙되는
    커밋"의 SSOT** 로 삼는 파일이라, 심링크만 되돌리면 `await-promotion.sh --check` 와
    `board:check` 가 **롤백으로 내린 커밋을 "prod 반영됨"으로 보고**한다. 보관 개수는
    `RELEASE_KEEP`(기본 3, 릴리스 1벌 ≈350MB 실측)이며 **현재 링크 대상은 어떤 경우에도
    지우지 않는다.**
  - **짝 갱신 대상 — 한 곳만 고치면 어긋난다.** 서빙 경로를 말하는 문서·스크립트가
    여럿이다: `infra/selfhost/README.md`(구성 요소 표 · 최초 부트스트랩 · 배포 파이프라인
    설명 · 프리뷰 잔여 사본 · 컷오버 origin 확인 절차) · `infra/selfhost/cutover.sh`
    Stage 5 의 산출물 검증 경로 · `preview.sh` 의 정리 대상. **P6 는 판단 규칙, README 가
    운영 좌표 정본**이라는 위 「셀프호스트 레인은 둘이다」 항목의 분담이 그대로 적용된다.
    ⚠️ 이 레포는 「같은 사실이 두 절에 흩어져 한쪽만 갱신돼 재착수 사고가 난」 이력이
    있다(위 Deployment Verification 절의 2026-08-21) — 서빙 경로를 또 바꾸게 되면 이
    목록을 먼저 훑을 것.
  - **판정은 앵커 계약 + 행위 계약 둘 다로 한다.** 이 결함은 배포와 요청이 겹쳐야만
    드러나는 경합이라 평시 테스트로는 영원히 초록이다. 그래서
    `selfhost-release-swap.contract.test.ts`(소스 앵커 — 구조가 유지되는가)와
    `selfhost-release-swap-behavior.test.ts`(`deploy.sh` 에서 해당 블록을 **발췌해 실제로
    실행** — 돌렸을 때 무슨 일이 일어나는가)를 짝으로 둔다. 🪤 **앵커만으로는 부족하다는
    것이 위 `mv -h` 건으로 증명됐다** — 그때 앵커 계약은 전부 초록이었다. 행위 계약은
    블록을 **원본에서 발췌**해 돌린다(손수 옮겨 적은 사본은 원본과 갈라져도 초록이 된다 —
    이 레포의 반복 결함).
...
- **작업 배칭 — 지금 아끼는 것은 "승격(빌드) 횟수"이지 머지 횟수가 아니다
  ...
  - **문서 전용 변경의 단독 PR 은 운영 빌드를 유발하지 않는다 — 다만 공짜도 아니다
    ...
    **레포가 public 이라 Actions 분은 무제한 무료다**"* 는 **SUPERSEDED** — 2026-08-26
    ...
    비용을 이유로 문서 PR 을 대기시키려면 **오너 재확인이 필요하다**.
    ⛔ **종전 안내 「잔여 분은 메뉴바 `actionsQuota` 행에서 본다」는 SUPERSEDED** — 그 행은
    2026-08-29 오너 결정으로 제거됐다(위 「Main Push Guard & Deploy CI Gate」의 같은 항목).
    ℹ️ **이 절의 계량 전제 자체가 08-28 공개 레포 이전으로 사라졌다** — 위 단가·총량
    수치는 다시 비공개로 돌릴 때만 판단 근거가 된다. 지금 확인이 필요하면
    `gh api "users/<owner>/settings/billing/usage?year=<Y>&month=<M>"` 를 직접 친다.
