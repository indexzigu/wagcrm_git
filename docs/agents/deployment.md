...
- **배포는 서빙 중인 산출물을 덮어쓰지 않는다 — 빌드 트리와 서빙 트리를 가른다
  ...
  - 🪤 **심링크 교체의 `mv` 에서 `-h` 를 빼면 조용히 실패한다.** 대상 `current` 가 이미
    디렉터리를 가리키는 심링크면 `mv` 는 링크를 **따라가** tmp 를 그 디렉터리 **안으로**
    옮기고 `exit 0` 을 낸다 — 링크는 옛 릴리스를 계속 가리키는데 배포는 성공으로 보인다.
    첫 배포만 우연히 맞고(대상 부재) 그 뒤로는 영영 갱신되지 않는다. 소스 리뷰로는
    안 잡혔고 **실행해 보고서야** 드러났다(4회 교체 후 current 가 1회차를 가리킴).
