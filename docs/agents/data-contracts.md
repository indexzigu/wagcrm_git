...
- **Progressive Lock Architecture (마감 캠페인 사후 취소 동기화):** 캠페인 마감(`isActive=false`) 이후 네이버에서 발생하는 '취소/반품'은 원본 스냅샷의 `cachedTotalRevenue` / `cachedDistinctOrderCount`를 직접 변조하거나 읽기 시점(Read-Path)에 라이브 재계산하지 않는다. 대신 `cachedPostCloseCancelQuantity`, `cachedPostCloseCancelRevenue` 필드에 사후 취소분만 따로 적어 두고 클라이언트에는 합산하여 반환한다.
  ...
  - **종료 조건 = 정산 락(2026-09-09).** 딜 하나라도 정산중·완료·드랍이면 그 캠페인은
    ...
    🪤 **이 종료 자체의 대가가 하나 있고, 그것이 돈 방향이다.** 실제 컷오프는 「락 시점」이
    아니라 **「직전 크론 실행 시점」**이라, 마지막 실행과 수동 락 사이에 들어온 취소는 값이
    이미 0 이 아니면 반영되지 않는다 — **취소 과소 계상 = 정산액 과대** 방향이다.
    그 창은 크론이 매일 성공할 때 24시간이고 **실패 회차가 끼면 늘어난다.** 영구는 아니고
    아래 `?includeLocked=1` 로 되돌릴 수 있으나 ⛔ **90일 창 안에서만이다**(대상 조회의
    `endDate` 필터는 그 옵션과 무관하다).
    ℹ️ 이 어긋남은 **의도(락=확정) 대비**이지 종전 동작 대비가 아니다 — 종전 대비로는 락
    이후의 취소도 함께 빠지고 그쪽은 오너 확정(2026-07-15)대로다.
    ℹ️ 아래 ①② 는 0-예외의 대가로 축이 다르다: ①은 이것과 **반대 방향**(락 이후 취소가
    반영됨)이고 ②는 방향이 아니라 **비용**(헛조회) 축이다. 값싼 후속안은 락으로 전이하는
    시점에 1회 재계산하는 것이다(별건).
    90일 창의 역할은 **둘로 갈렸다**: 락이 걸리는 캠페인에는 정산 락이 1차 통제이고 이
    창은 「끝내 락되지 않는 건」의 백스톱이지만, 아래 ② 때문에 **취소가 0 인 락 캠페인**
    에는 이 창이 여전히 유일한 1차 통제다.
    🪤 **예외: 두 값이 모두 0 이면 락이어도 계속 조회한다.** 기본값이 0 이라 "계산했는데
    취소가 없었다"와 "계산된 적이 없다"가 구분되지 않는데, 마감과 정산 착수가 같은 날이면
    하루 1회인 크론이 그 사이에 끼지 못해 0 이 영구가 되고 **유일 writer 라 되돌릴 길이
    없다.** 그래서 틀리는 방향을 헛조회(90일 창이 상한) 쪽으로 잡는다.
    ⛔ "락이면 무조건 건너뛴다"로 단순화하지 말 것.
    ⚠️ 대가 둘을 함께 적어 둔다: ①**동결 시점이 그 부분집합에선 「락 시점」이 아니라 「처음
    0 이 아닌 값이 나온 시점」**이다(건너뛰지 않는다는 것은 다시 **쓴다**는 뜻이다)
    ②**수렴하지 않는다** — 취소가 정말 0 인 건은 90일 창이 끝날 때까지 매일 재조회된다.
    위 종료 자체의 대가까지 **셋을 함께 닫는** 근본 처방은 「마지막 계산 시각」 컬럼이며
    스키마 변경이라 별건이다.
    수동 재계산 레버 `?includeLocked=1` 은 그 위의 보조이고, 호출 경로는
    `run-cron.sh 'naver-settlement-sync?includeLocked=1'`(잡 이름이 URL 에 그대로 이어 붙는다)
    또는 수동 curl 이다 — ⛔ 레이더 실행 버튼은 쿼리를 안 붙여 못 쓴다.
