...
- **Campaign Period SSOT = 판매관리 일정 (오너 확정 2026-07-15, 기존 규칙 개정):** 집계 창의 정본은
  **판매관리(`SalesCampaign`) 일정**이고, 네이버 스토어 기간(`OrderCampaign.salePeriod`)은 **관측값**이다.
  ...
  - **어긋남은 드러낸다 — 두 번째 경로 추가(오너 결정 2026-09-17).** 위 규칙의 대가는 **스토어에서 기간을
    바꿔도 화면에 아무 흔적이 남지 않는다**는 것이었다. 실측(2026-09-17): 프로덕션 활성 주문캠페인은 전부
    판매캠페인 연결이 있어 `salePeriod` 가 화면에 도달할 경로가 **하나도 없었고**, 오너는 그 상태를
    "등록 당시 기간으로 고정돼 있다"고 신고했다. 즉 결함은 재동기화가 아니라 **관측값의 종착지가 없다**는
    것이었다. 정본은 그대로 두고(자동 추종은 여전히 금지 — '종료 후 임시 오픈'이 회차 창으로 흘러든다)
    판정 `resolveStorePeriodDrift`(sale-window SSOT)가 스토어 기간과 집계 창을 **같은 포맷터를 통과한
    문자열**로 비교해 어긋남과 **맞출 대상 회차**를 함께 내고, 주문관리 카드가 배지 + 한 번 누르는
    액션으로 노출한다(창이 얼었으면 캠페인 전체를 뺀다 — 아래 ⚠️). 액션은 **기존 캠페인 PATCH 를 그대로 탄다**(그룹 팬아웃·롤업·단축링크
    만료·구글 캘린더 `after()` 훅이 거기 달려 있다) — ⛔ 전용 쓰기 경로를 만들지 말 것.
    ⛔ 여러 회차를 **동시에** PATCH 하지 말 것: 그 라우트는 그룹 공유 일정 팬아웃과 멤버십 낙관 검사를
    한 트랜잭션에서 하므로 형제 회차를 나란히 보내면 409 가 나거나 뒤덮인다(순차 실행이 계약).
    ⛔ **판매가 끝난 상태의 스토어 기간은 시작일을 쓰지 말 것 — 종료일만 맞춘다**(오너 결정 2026-09-17,
    실사고에서 나왔다). 네이버는 상품이 `CLOSE`·`OUTOFSTOCK` 이 되면 판매기간을 **종료일 기준으로 다시 써서**
    돌려준다. 프로덕션 실측 2건: 2주짜리 회차가 `07.20 ~ 07.20`(하루)로, `09.10~09.16` 회차가
    `09.16 ~ 09.17`(**시작일 = 원래 종료일**)로 왔다. **시작일만 못 쓰는 값이 되고 종료일은 실제와 맞았다.**
    실사고: 그 값을 판매관리에 반영해 회차 시작일이 `09.10 → 09.16` 으로 망가졌다(오너가 발견, 수동 정정).
    - 판정 SSOT 는 `STORE_PERIOD_TRUSTED_STATUS`(= `SALE`) 하나이고 `resolveStorePeriodDrift` 가
      `scope: 'full' | 'end-only'` 로 답한다. `end-only` 면 `storeStartYmd` 가 **null 이고 PATCH 에 시작일을
      싣지 않는다** — 그 null 이 이 사고를 막는 지점이다.
    - ⛔ **`end-only` 에서 시작일 차이를 어긋남으로 세지 말 것.** 세면 눌러도 사라지지 않는 **영구 배지**가
      되고, 누르면 다시 시작일이 망가진다. 종료일이 같으면 신호를 내지 않는다.
    - **판매중이어도 스토어 시작일 전에 이미 매출이 있으면 `end-only` 다(오너 결정 2026-09-24).** 회차는
      첫 매출일에 이미 시작한 것이라, 스토어가 기간을 뒤로 다시 잡았다고 시작일을 옮기면 그 사이 매출이
      집계 창 밖으로 빠진다. 입력은 유효 주문 중 가장 이른 결제 시각(`firstSaleMs`)이고, 비교는 KST 날짜
      단위다. 이유는 `startKeptReason`(`store-closed` | `sales-before-store-start`)으로 구분해 팝오버 문구를
      가른다. 매출이 없거나 스토어가 시작일을 앞당긴 경우는 종전대로 `full`.
    - ⛔ **신뢰 상태를 넓히지 말 것**(모름·`WAIT`·`SUSPENDED` 도 전부 `end-only`). 같은 계열의 선례가 이미
      있다 — `campaigns-handler` 가 `WAIT`·`SUSPENDED`·`OUTOFSTOCK` 의 14일짜리 기본 기간을 '미등록'으로
      거른다(네이버가 기간을 합성하는 또 다른 모양). 🪤 **마감(`isActive=false`) 캠페인의 `salePeriod` 는
      동결돼 있어 이 현상이 안 보인다** — 그 행들이 멀쩡하다는 것을 반증으로 읽지 말 것(마감 전 관측값이다).
    ⚠️ **딜 하나라도 정산 락이면 이 캠페인 전체를 뺀다**(호출부의 `periodFrozenBySettlement` 와 같은
    판정이라 `resolveStorePeriodDrift` 가 함께 갖는다 — 두 곳에 두면 갈리고, 호출부가 선차단하면
    함수 안의 필터가 **도달 불가 코드**가 되어 그걸 고정한 테스트가 프로덕션에서 발화하지 않는
    초록불이 된다). 미락 회차만 골라 맞춰 봐야 **집계 창이 얼려 있어 화면이 움직이지 않고**, 배지는
    눌러도 사라지지 않는 무한 루프가 된다.
    ⛔ **그 구간의 스토어 변경은 어느 표면에도 뜨지 않는다 — `periodFrozenDrift` 가 덮는다고 적지 말 것.**
    그쪽 판정은 `windowDiffers && periodFrozenBySettlement` 이고 `windowDiffers` 는 **판매관리 일정 ↔ 저장
    창** 차이라, 스토어만 바뀐 경우는 `false` 다. 즉 락 회차에서는 이 신호가 **정말로 없다**.
    ✅ **오너 확정 2026-09-17: 이대로 둔다.** 정산이 시작된 회차는 기간이 확정된 것이라 바꾸면 정산 내역과
    어긋난다. ⚠️ **다만 이 상태는 드물지 않다** — 끝난 지난 회차의 판매캠페인이 정리되지 않고 붙어 있으면
    (2026-09-16 실사고와 같은 모양: 2차 주문캠페인에 1차 회차 3건이 완료·정산중 상태로 남아 있었다) **살아
    있는 캠페인인데도 배지가 영영 안 뜬다.** 오너 처방은 **그 캠페인에서 끝난 회차의 연결을 떼는 것**이고,
    ⛔ 「끝난 회차를 창 동결 판정에서 빼는」 코드 우회로 해결하지 말 것(그건 2026-07-15 결정을 바꾸는 일이라
    정산 중인 주문 귀속에 영향을 준다 — 조회창 전용 `resolveQueryWindowContributors` 와 혼동 금지).
  - **재동기화 창은 두 구간이다(오너 결정 2026-09-17, 종전 단일 창 대체).** 종료 임박(리드 2일)~경과
    (그레이스 7일) 구간은 종전대로 매 조회 후보이고, **종료가 아직 먼 구간은 `PERIOD_RESYNC_IDLE_INTERVAL_MS`
    (4시간)에 1회**만 묻는다. 종전에는 그 구간을 **아예 묻지 않아** 2주짜리 캠페인 3일차의 기간 연장·단축을
    종료 이틀 전까지 관측조차 못 했다. 판정 SSOT 는 `mapping-service` 의 `shouldResyncCampaignPeriod` ·
    `usesIdlePeriodCheckInterval`(둘 다 같은 구간 분류를 공유한다 — 리드 경계를 한쪽만 고치면 **찍는 구간과
    읽는 구간이 어긋난다**).
    - **확인 시각 `OrderCampaign.periodCheckedAt` 은 유휴 구간에서만 찍는다** — 임박 구간의 판정은 그 값을
      읽지 않으므로 거기서 찍으면 대시보드 GET 마다 캠페인 수만큼 쓰기만 는다(egress 규율).
    - ⛔ **만기가 된 캠페인에만 찍지 말 것.** 그러면 캠페인마다 만기 시각이 어긋난 채 남아 **캠페인 수만큼
      스토어 호출이 따로 난다**(위상 분산 — 60초 TTL 은 동시 진입만 합칠 뿐 몇 시간짜리 위상차는 못 합친다).
      **유휴 만기가 하나라도 있는 회차**에 유휴 구간 **전원**을 함께 찍어 위상을 모은다(응답이 이미 손에
      있어 추가 호출 0).
    - ⛔ **조회가 났다는 이유만으로 찍지 말 것(짝 조건).** 임박·`unset` 캠페인이 하나라도 있으면 조회는
      **매 GET** 나는데 거기에 얹으면 유휴 캠페인 전원이 매 GET 쓰기를 받는다 — 임박 구간을 기록에서 뺀
      바로 그 이유가 유휴로 옮겨갈 뿐이다. 그 회차는 추가 호출을 만든 주체도 아니라 위상을 모을 이유가 없다.
    - ⚠️ **이 짝이 묶는 것은 「유휴 레인」뿐이다.** 임박 구간과 `unset`(기간 미확정·'~ 계속')은 설계상 매 GET
      후보이므로, 그런 캠페인이 하나라도 있으면 스토어 조회는 그 캠페인이 부른다 — **"4시간에 한 번"을 전체
      상한으로 읽지 말 것.** 유휴 레인이 추가로 만드는 호출의 상한이라는 뜻이고, 그마저 **평시값이다**:
      위상에서 벗어난 캠페인은 그 회차에 1회를 더 만들고 전원의 위상을 그 시각으로 재설정한다. 두 부류다 —
      `periodCheckedAt` 이 **null**(신규 등록분)과 **낡은 값**(기간이 연장돼 임박→유휴로 되돌아온 건. 유휴에서
      찍힌 적이 있으면 null 이 아니라 옛 시각이다). 어느 쪽이든 즉시 만기라는 결론은 같다.
      배선 계약은 `campaigns-handler.period-check.test.ts`.
    - ⛔ **스토어 조회가 실패한 회차에는 아무도 찍지 않는다** — 확인하지 않은 것을 확인으로 굳히면 다음
      간격까지 기간 변경을 놓친다(P0 No Silent Failure).
...
- **`searchNaverProducts` = 60초 TTL 쿨다운 (측정 후 결정, 2026-07-30):** 이 조회는
  ...
  - **실측(30일):** 총 776회 · 활성일 16~192회/일 · 비활성일 **0회**(재동기화 창 게이트가
    ...
    ⚠️ **이 실측의 전제는 2026-09-17 에 바뀌었다** — 위 *Campaign Period SSOT* 의 「재동기화 창은 두
    구간이다」가 유휴 구간(종료가 먼 활성 캠페인)을 4시간 간격으로 후보에 넣었으므로, **활성 캠페인이
    있는 날의 "비활성일 0회"는 더 이상 성립하지 않는다.** 대신 유휴 구간의 상한은 게이트가 아니라
    그 간격 + 위상 모으기가 정한다. 새 기준선이 필요하면 아래 Sentry `dataset=spans` 경로로 다시 잰다.
  ...
  - ⛔ **TTL 을 재동기화 창에 근접하게 키우지 말 것.** 60초가 안전한 이유는 창
    (`shouldResyncCampaignPeriod`)의 **가장 짧은 구간**이 리드 2일짜리라서다 — 이 조회의 존재
    이유("스토어에서 기간이 연장됐는데 판매관리엔 없다" 감지, 58 vs 발주 78 실사고의 축)는
    그대로 유지된다. ⚠️ **TTL 을 유휴 간격(4시간)의 절감 수단으로 쓰지 말 것** — TTL 은 같은
    60초 안의 동시 진입만 합치고, 유휴 구간의 상한은 `periodCheckedAt` 위상 모으기가 정한다
    (위 *Campaign Period SSOT*). 계약은 `naver-commerce-api.test.ts` 가 고정한다(특히 "TTL 이
    지나면 다시 조회한다").
...
- **Pending-Order Fetch Window = order-fetch-window SSOT (2026-07-30):** 발주 대상 조회의
  ...
  - **끝난 회차는 창 시작일에 기여하지 못한다(2026-09-16 개정).** 판매관리 창 후보에서 정산
    락(`isSalesCampaignLocked` — 정산중·완료·드랍)이 걸린 회차를 먼저 걷어낸다. 창이 연결된
    판매캠페인 시작일의 **최솟값**이라, 2차 주문캠페인에 1차 회차가 연결된 채 남아 있기만 해도
    창이 그만큼 앞으로 끌린다 — 실사고에서 캠페인 기간이 9/10~9/16 인데 주문확인이 **6/12 부터
    97일**을 훑었다(네이버 조회 13회 + 생략 84일 · 53초). 증상이 "주문확인이 느리다" 뿐이라
    조용히 누적된다. ⛔ **판정 집합을 새로 선언하지 말 것** — 오너가 이미 "정산중·완료·드랍이면
    그 회차는 확정이므로 더 조회하지 않는다"로 정했다(T-140 Progressive Lock). 마감(`CLOSED`)·
    정산대기는 **빼지 않는다**(판매 종료 직후 결제분이 아직 발주 대상이다).
    ⚠️ **전부 끝난 회차면 아무것도 걷어내지 않는다** — 후보가 0이 되면 창이 저장 창이나 기본
    창(오늘-14일)으로 떨어져 발주서에서 주문이 조용히 빠진다(P0). 좁히는 것은 살아있는 회차가
    창을 붙들고 있을 때뿐이다. 같은 이유로 `status` 가 없으면(select 누락) **좁히지 않는다** —
    그 fail-safe 때문에 select 누락이 크래시 없이 게이트만 죽이므로
    `query-window-status-select.contract.test.ts` 가 두 라우트의 select 를 고정한다.
    ⚠️ 이 필터는 **조회창 전용**이다 — 집계 창(`resolveSalesCampaignWindow`)은 "연결된 전부를
    min~max 로 합성하되 어긋나면 경고"가 오너 결정이라 그대로 둔다.
...
- **Progressive Lock Architecture (마감 캠페인 사후 취소 동기화):** 캠페인 마감(`isActive=false`) 이후 네이버에서 발생하는 '취소/반품'은 원본 스냅샷의 `cachedTotalRevenue` / `cachedDistinctOrderCount`를 직접 변조하거나 읽기 시점(Read-Path)에 라이브 재계산하지 않는다. 대신 `cachedPostCloseCancelQuantity`, `cachedPostCloseCancelRevenue` 필드에 사후 취소분만 따로 적어 두고 클라이언트에는 합산하여 반환한다.
  ...
  - **종료 조건 = 정산 락 + 확정 마커(2026-09-09 / T-140 개정).** 딜 하나라도 정산중·완료·
    드랍이면 그 캠페인은 확정이므로 더 조회하지 않는다(판정 SSOT `isSalesCampaignLocked` —
    정산대기까지는 변동). 단 **"이미 확정 계산을 마쳤는가"는 값이 아니라
    `OrderCampaign.cachedPostCloseCancelFinalizedAt`(nullable DateTime)가 답한다.**
    - **왜 마커가 필요한가:** 두 값이 `Int? @default(0)` 이라 "계산했는데 취소가 0" 과
      "계산된 적이 없다" 가 값만으로는 구분되지 않는다. 유일 writer 라 0 으로 굳으면
      되돌릴 길도 없다.
    - **마커의 의미는 「마지막 계산 시각」이 아니라 「락 상태에서 계산해 확정한 시각」이다.**
      락이 아닌 동안의 계산에는 찍지 않고, **락이 풀리면 그 회차에 지운다**(안 지우면
      되돌린 캠페인이 재락 시 확정 계산 없이 옛 값으로 굳는다).
    - **락 캠페인은 락 이후 첫 성공 실행에서 정확히 한 번 계산된다** — 그 계산이 실행
      시각까지의 취소를 전부 담는다.
    - ⚠️ **`cachedProductOrderIds` 가 비면 계산도 확정도 하지 않는다** — "주문 0 건" 과
      "마감 스냅샷이 주문을 못 담았다" 가 구분되지 않아 그대로 확정하면 0 이 영구가 된다.
      네이버 호출이 0 이라 매일 재평가해도 비용이 없다.
    - ⛔ **값이 0 인지로 미계산을 판별하던 종전 방식으로 되돌리지 말 것.**
  - **T-140 이 닫은 것 — 종전 값 기반 판별이 남긴 한계 셋:**
    ①**동결 시점이 부분집합마다 달랐다**(0/0 예외로 건너뛰지 않는 부류는 「락 시점」이 아니라
    「처음 0 이 아닌 값이 나온 시점」에 얼었다) ②**수렴하지 않았다**(취소가 정말 0 인 캠페인은
    90일 창이 끝날 때까지 매일 재조회) ③**과소 계상 = 정산액 과대**(컷오프가 「락 시점」이
    아니라 「직전 크론 실행 시점」이라 마지막 실행과 수동 락 사이의 취소를 놓쳤고, 크론
    실패 회차가 끼면 그 창이 늘어났다). 마커는 셋을 함께 닫는다 — ①②는 확정 판정이 값과
    무관해지면서, ③은 확정 계산이 **락 이후**에 돌면서.
  - **⛔ 락 전이 훅을 새로 만들지 말 것 — 이 설계에서는 불필요하다.** 확정 계산이 락 이후
    첫 실행에 붙어 있으므로 ③의 누락 구간이 없다. `SalesCampaign.status` 를 쓰는 경로가
    라우트·리포지토리·lib 에 흩어져 있어 전이 훅은 그 전부에 배선해야 하는데, 마커 방식은
    배선 0 으로 같은 결과를 얻는다(T-140 판단).
    ⚠️ **단 확인 기간 안에서만이다**(`post-close-check-window.ts` — 전 주문 종결을 처음 본 날 +10일,
    종결을 못 본 캠페인은 판매 종료 +15일) — 기간이 끝난 뒤 락이 걸리는 캠페인은 확정 계산이 없고
    기간 안 마지막 값이 최종이다(오너 확정 2026-09-11: 그 뒤 건은 정산 때 수동 조정).
  - 🔒 **확정의 전제 하나 더 — 응답이 온전해야 한다.** `queryOrderDetails` 는 응답이 요청보다
    모자라도 `console.warn` 만 남기고 짧은 배열을 돌려준다. 그 결과로 확정하면 일시적 API
    저하가 **과소 계상된 값을 영구히 동결**시킨다(유일 writer 라 수동 레버 말고 복구 경로가
    없다 — ⚠️ **그 레버조차 응답이 모자라는 동안은 무위다**, 아래 레버 불릿 참조).
    그래서 요청한 id 가 전부 돌아오지 않으면 **값은 쓰되 확정은 미룬다** — 미룬 건수는
    `deferredIncomplete` 로 응답에 싣는다(P0 No Silent Failure).
    ⛔ **완전성을 「개수」로 재지 말 것** — 중복·잉여 행 하나가 **빠진 id 를 가려 「온전함」
    으로 읽힌다.** 판정은 요청 id **집합**이 전부 돌아왔는가로 한다. 🪤 `normalizeQueriedOrder`
    는 `productOrder` 가 **있기만 하면 통과**시키므로(버리는 것은 `productOrder` 자체가 없을
    때뿐이다) **id 없는 행도 배열에 남는다** — 그런 행은 집합에 안 들어와 「모자람」으로
    판정된다(fail-closed, 의도한 방향).
    ⛔ **이미 확정된 캠페인은 응답이 모자라면 값도 쓰지 않는다**(🔒-b). 그대로 쓰면 온전했던
    값이 과소 계상 값으로 바뀌는데 마커는 남아 있어 다음 회차부터 다시 건너뛴다 — 이 게이트가
    막으려던 영구 동결이 `includeLocked` 강제 재계산 경로로 되살아난다(2회차 리뷰가 잡았다).
    그 건너뜀은 `protectedFinalized` 로 따로 센다(`deferredIncomplete` 와 배타적 — 「값은
    갱신됐다」와 「아무 것도 안 썼다」를 한 숫자로 합치면 오너가 레버 결과를 오독한다).
    ℹ️ **미락 캠페인에는 이 보호를 걸지 않는다** — 값이 애초에 얼어 있지 않고, 마커는 오히려
    지워야 재락 시 확정 계산을 받는다.
    ⚠️ **탈퇴 구매자의 주문은 커머스API 가 영구히 제공하지 않으므로**(위 *Product-Order Query
    Paging* 의 구조적 원인 ②) 그런 주문이 섞인 캠페인은 이 전제를 영영 못 채우고(종결도 못 본다)
    판매 종료 +15일까지 매일 재조회된다 — 알고 택한 값이다(틀리는 방향을 「헛조회」 쪽으로 잡는다).
  - ⚠️ **마커 삭제는 「관측했을 때」만 일어난다.** 두 크론 실행 **사이에** 락 해제 → 재락 을
    마치면 이 잡은 해제를 못 보고 옛 마커로 건너뛴다. 그 경우 값을 다시 받으려면
    `includeLocked` 레버를 쓴다 — 배선으로 닫으려면 status 를 쓰는 경로 전부에 마커 무효화를
    넣어야 해서 위 「전이 훅 불필요」와 같은 비용 판단으로 문서화를 택했다(T-140).
  - ⚠️ **대신 남는 어긋남 하나(방향이 반대다).** 확정 계산이 락 **이후**에 돌므로
    「락 ~ 그 실행」 사이에 들어온 취소는 **포함된다** — 의도(락=확정) 대비 취소 과다 계상이다.
    위 ③과 반대 방향이고, 그 창은 다음 성공 실행까지이며(크론 실패가 끼면 늘어난다)
    **한 번 확정되면 더 벌어지지 않는다** — ③은 반대로 실패가 낄수록 누락이 커졌다.
    ℹ️ 이 어긋남은 **의도 대비**이지 종전 동작 대비가 아니다.
  - ⚠️ **이 두 값을 「정산액」으로 말하지 말 것(2026-09-09 실측 정정).** 소비처는
    `campaigns-handler` → 마감 캠페인 판매 리포트·보드 카드의 **표시**뿐이고
    (`cancelReturnQuantity`/`cancelReturnAmount`), 정산 금액 계산에 들어가지 않는다 — 그
    리포트의 총매출·총수량은 이미 취소·반품이 빠진 순수치라 이 값은 「차감」이 아니라
    「참고」로 붙는다(`SalesReportModal` 주석). 위 ③의 **「= 정산액 과대」 표현은 그만큼
    과장이었다** — 실제 피해는 오너가 보는 참고 수치가 틀리는 것이다. 오차의 방향 서술은
    유지하되 돈 계산에 걸린 것처럼 읽지 말 것.
  - **확인 기간(오너 확정 2026-09-11):** 마감 후 취소·교환 확인은 하루 1회, 전 주문 종결
    (PURCHASE_DECIDED·CANCELED·CANCELED_BY_NOPAYMENT·RETURNED·EXCHANGED, 또는 DELIVERED + 클레임이
    완료·거부로 끝남)을 **처음 본 날**(`OrderCampaign.cachedPostCloseAllTerminalAt`) **+10일**(KST 날짜)
    까지다 — 판매 종료일과 무관하다(공구는 판매 종료 뒤 배송이 흔하다). 종결을 한 번도 못 본
    캠페인(탈퇴 구매자 주문 · 추적 정지)만 **판매 종료 +15일**에서 멈춘다(오너 원문 「배송완료로부터
    최대 15일」을 배송완료를 끝내 못 보는 부류에 옮긴 해석 — 오너 승인). 종전 90일 창은 구 레포에서
    넘어온 상수로 오너 결정 기록이 없었고 이 규칙이 대체했다. 판정 SSOT `post-close-check-window.ts`.
    ⛔ 클레임 종결을 claim-derive `isCompleted`(부분 문자열 'DONE')로 판정하지 말 것 — `COLLECT_DONE`
    (수거만 끝남)을 완료로 읽는다. ⚠️ 목록 밖·미확인 클레임 상태는 진행 중으로 읽혀 종결 시각이 막히고
    판매 종료 +15일 상한으로 넘어간다 — 늦게 배송된 캠페인은 종결 +10일보다 일찍 멈출 수 있다(예외는
    수동 조정). ⛔ 창을 넓혀 「안전」을 사지 말 것 — 곧 Fixie 상시 호출량이다(캠페인당 하루
    ceil(주문/300) 요청). 기간 안에서는 락 캠페인을 여전히 확정 마커가 먼저 건너뛴다.
  - **관측 카운터:** `campaigns`(후보 창 — 판매 종료 +26일 — 안 전체) · `skippedLocked`(확정돼 건너뛴 건) ·
    `finalizedLocked`(이번 실행에서 확정된 건 — 배포 후 따라잡기의 진행과 수렴을 재는 신호) ·
    `deferredIncomplete`(응답이 모자라 **확정만** 미룬 건 — 값은 갱신됐다. 매일 0 이 아니면
    그 캠페인은 수렴하지 못하고 있다) · `protectedFinalized`(응답이 모자라 **확정된 값을 지키느라
    통째로 건너뛴** 건 — 값도 마커도 안 썼다) · `updated`(값이 실제로 바뀐 건) ·
    `stoppedAfterTerminal`(종결 +10일이 지나 조회 없이 멈춘 건) · `stoppedByBackstop`(종결을 못 본 채
    판매 종료 +15일을 넘어 멈춘 건) · `markedAllTerminal`(전 주문 종결을 처음 관측한 건) ·
    `clearedAllTerminal`(움직이는 주문이 다시 보여 종결 시각을 지우고 확인을 연장한 건).
    ⚠️ 앞의 두 카운터는 **락 캠페인만** 센다 — 미락 캠페인의 모자란 응답은 어느 쪽에도 안
    잡힌다(미락은 확정 대상이 아니라 다음 회차가 그냥 다시 계산한다). ⛔ 종전 `requeriedUncomputed` 는 **제거됐다**(모집단이던
    「락인데 값이 0」이라는 부류 자체가 사라졌다).
    ⚠️ 배포 후 첫 실행은 기존 행의 마커가 전부 null 이라 **창 안의 락 캠페인이 1회씩
    확정 계산을 한다**(의도된 따라잡기). 그 회차의 `finalizedLocked` 가 그 수다.
    ⚠️ `cron.log` 의 성공 줄은 잘릴 수 있다 — 판정은 `SystemTaskLog.details` 를 직독한다.
  - **수동 재계산 레버 `?includeLocked=1`** — 확정된 값이 틀렸다고 판단될 때 강제로 다시
    계산한다. ⚠️ **단 응답이 모자라는 동안 이 레버는 무위로 끝난다** — 위 🔒-b 가 확정된 값을
    지키려고 통째로 건너뛰기 때문이다(값도 마커도 안 쓴다). 그 회차의 `protectedFinalized` 가
    그 사실을 보고하므로 「눌렀는데 아무 일도 안 일어났다」가 조용히 지나가지는 않는다.
    ⛔ 이걸 「레버가 고장났다」로 읽고 가드를 풀지 말 것 — 푸는 순간 온전했던 값이 과소 계상
    값으로 덮이고 마커가 남아 영구 동결이 재발한다(2회차 리뷰가 잡은 바로 그 경로). 호출 경로는 `run-cron.sh 'naver-settlement-sync?includeLocked=1'`(잡 이름이
    URL 에 그대로 이어 붙는다) 또는 수동 curl이다 — ⛔ 레이더 실행 버튼은 쿼리를 안 붙여
    못 쓴다. ⛔ **확인 후보 창 안에서만이다** — 종결을 못 본 캠페인은 판매 종료 +15일,
    종결을 본 캠페인은 판매 종료 +26일까지다(종결 +10일 중단과 확정 건너뛰기는 레버가 넘는다).
