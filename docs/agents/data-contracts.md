...
- **Progressive Lock Architecture (마감 캠페인 사후 취소 동기화):** 캠페인 마감(`isActive=false`) 이후 네이버에서 발생하는 '취소/반품'은 원본 스냅샷의 `cachedTotalRevenue` / `cachedDistinctOrderCount`를 직접 변조하거나 읽기 시점(Read-Path)에 라이브 재계산하지 않는다. 대신 `cachedPostCloseCancelQuantity`, `cachedPostCloseCancelRevenue` 필드에 사후 취소분만 따로 적어 두고 클라이언트에는 합산하여 반환한다.
  ...
  - **관측 카운터:** `campaigns`(창 안 전체) · `skippedLocked`(확정돼 건너뛴 건) ·
    ...
    `deferredIncomplete`(응답이 모자라 확정을 미룬 건 — 매일 0 이 아니면 그 캠페인은 수렴하지
    못하고 있다는 뜻이다) · `updated`(값이 실제로 바뀐 건). ⛔ 종전 `requeriedUncomputed` 는 **제거됐다**(모집단이던
  ...
  - **수동 재계산 레버 `?includeLocked=1`** — 확정된 값이 틀렸다고 판단될 때 강제로 다시
    계산한다. 호출 경로는 `run-cron.sh 'naver-settlement-sync?includeLocked=1'`(잡 이름이
