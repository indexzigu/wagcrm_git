...
- IMAP 에 붙는 파일은 전부 `resolveImapConfig` 를 **거친다** — connect 인자가 SSOT 호출에서
  온 것인지를 **AST 로** 본다. ⛔ 정규식으로 되돌리지 말 것: 이 계약은 정규식으로 세 번
  뚫렸다(①`resolveImapConfig` 를 언급만 해도 통과 ②`port`·`tls` 키 부재로 재니 정작
  `tlsOptions:` 가 안 걸려 거짓 음성 ③`g` 플래그 누락으로 **파일당 첫 connect 만** 검사).
  지금은 스프레드 덮어쓰기·변수 호이스트·둘째 호출·`new Imap(` 네 형태를 전부 잡는다(변이 확인).
