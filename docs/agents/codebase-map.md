...
| `resolveImapConfig` · `resolveSmtpConfig` · `orderMailboxesForScan` | `src/lib/mail-config.ts` | 메일 서버 **좌표·편지함 순회 정책** SSOT. 소비처 셋(`tax-invoice-mail/mail-scan.ts` 수취 계산서 스캔 · `order-converter/api/fetch-emails` 송장 회신 · `order-converter/api/send-email` 발주 발송)이 **같은 계정 자격증명**(`SMTP_USER`/`SMTP_PASS`)을 공유하는데 호스트만 각자 리터럴로 박혀 있었다 — 그 상태에서 계정을 옮기면 고칠 곳이 셋인데 **한 곳을 빠뜨려도 타입도 테스트도 못 잡는다**(그 기능만 옛 서버에 새 자격증명으로 붙어 인증 실패). 2026-09-01 다음메일→구글 전환의 계기다. ⛔ 소비처에 호스트를 다시 적지 말 것 — `mail-config.contract.test.ts` 가 소스 스캔으로 막는다(🪤 주석을 걷어내고 스캔한다: 이 트랙의 문서가 옛·새 서버 이름을 **설명하려고** 인용한다. 양성 프로브로 SSOT 자신에는 매치가 있음을 함께 단언해 스캐너 고장이 초록으로 보이는 것을 막는다). ⚠️ **구글은 일반 비밀번호를 안 받는다**(2025-03-14 차단) — `SMTP_PASS` 는 앱 비밀번호이고, OAuth 는 메일함 전체 접근이 제한 범위 스코프라 CASA 감사가 붙어 기각했다. ⚠️ **발신인 치환이 침묵형이다**: `SMTP_FROM_EMAIL` 을 구글에 「다른 주소에서 메일 보내기」로 등록하지 않으면 오류가 아니라 **로그인 계정 주소로 조용히 바뀌어** 나간다(로그는 성공, 브랜드사 화면에서만 드러난다) — `fetch-emails` 의 `isMyOwnMail` 이 로그인 계정 주소도 함께 보는 이유다(안 보면 우리가 보낸 발주서 원본이 회신으로 오인된다). 🪤 **편지함 제외를 이름 목록으로 다시 만들지 말 것**: 종전 인라인 목록은 다음메일의 **띄어쓴** 한국어 이름(`지운 편지함`)만 알아서 구글의 `휴지통`·`보낸편지함`·`전체보관함` 이 하나도 안 걸렸다(전체보관함은 전 메일의 사본이라 메일함을 두 번 훑는다). 판정은 ①IMAP 특수용도 속성 ②공백을 걷어낸 이름 폴백 두 겹이다. ℹ️ **`전체보관함`은 제외가 아니라 맨 뒤로 미룬다** — 오너가 회신을 읽고 보관하면 그 메일은 거기에만 남으므로 빼면 영영 「회신 없음」이 되고, 호출부가 첫 발견에서 순회를 멈추므로 앞에서 찾으면 비용이 0 이다. 오너 절차는 `docs/runbooks/gmail-mail-cutover.md` |
