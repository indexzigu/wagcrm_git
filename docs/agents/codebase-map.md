...
- IMAP 에 붙는 파일은 전부 `resolveImapConfig` 를 **거친다** — 🪤 「언급」만 보면 옵션을 손수
  지어 넘기는 위조가 통과하므로 connect 인자가 SSOT 호출에서 온 것인지까지 본다.
- 🪤 **주석을 걷어내고 스캔하되 `//` 앞의 `:` 는 지킨다** — 가드 없이 자르면
  `"imaps://imap.example.net"` 같은 URL 형태 설정이 통째로 잘려 위반이 사라진다.
...
(`https://mail.google.com/`)은 구글의 **제한 범위 스코프**라 CASA Tier 2 보안 감사가 필요하고
(자가진행 경로도 $540~1,000) 이 작업 규모에 비해 과하다. 다시 제안하기 전에 그 비용부터 확인할 것.
...
`휴지통`·`보낸편지함`·`전체보관함`·`중요`가 하나도 안 걸렸다(전체보관함·중요는 다른 폴더의
