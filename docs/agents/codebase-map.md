...
| `refreshCampaignRows` · `LIST_REFRESH_FAILED_MESSAGE` | `src/lib/campaign-row-refresh.ts` | 쓰기 직후 **캠페인 행을 다시 읽어 화면에 되꽂는** 동작의 SSOT(클라이언트 전용). 상위 콜백(`onCampaignUpdated`→`replaceCampaignRow`)이 **행 하나를 교체하는** 계약이라 목록이 스스로 따라오지 않아서, 쓰기를 한 화면마다 「무엇을 다시 읽을지」를 각자 정해 왔고 **사본이 갖춘 조각이 서로 달라졌다** — 정산 패널만 응답 모양을 검증했고(`"id" in body`) 그룹 섹션·대시보드는 캐스팅만 했으며, 실패 통지는 넷 중 둘에만 있었다(합류는 반환값을 통째로 버렸다). ⛔ **문구는 이 모듈이 갖지 않는다** — 목록 배지가 낡는 것과 패널 칸이 빈 채로 남는 것은 사용자가 취할 행동이 달라, 통일하면 한쪽이 반드시 틀려진다. 보드 표면 넷이 공유하는 한 문장만 상수로 둔다. ⚠️ **흡수 목록은 전수가 아니다** — `campaign-side-panel` 의 `refreshCampaignSnapshot` 은 실패 시 로컬 병합으로 폴백하는 **다른 계약**이라 일부러 두었다(흡수하면 그 폴백이 사라진다). ⚠️ 그룹 멤버십이 바뀌면 **바뀐 행 전부**를 넘긴다 — 제외는 「제외 전 멤버 전원」(해체는 소속이, 존속은 남은 형제의 `groupMemberCount` 가 바뀐다), 합류는 응답이 싣는 멤버 전원. 이것이 성립하려면 `CAMPAIGN_DETAIL_INCLUDE` 가 `group._count` 를 실어야 한다(T-100 — 없으면 형제를 다시 읽을 때 숫자가 고쳐지는 게 아니라 **사라진다**). 계약은 `campaign-row-refresh.contract.test.ts`(주석 제거 + 상수 파생 needle + 양성/음성 프로브) · 행위는 `campaign-row-refresh.test.ts` |
