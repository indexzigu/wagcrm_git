...
- **주요 경로:** `src/app`(페이지 + `api/`) ·
  ...
  **`ygrd-link/`(별도 배포체 — Cloudflare Worker, 유입추적 리다이렉터. Next 앱이
  ...
  `knowledge/`(결재 규칙 `rules/approval.rules.json` 만 남음 — 채팅 매니페스트
  `index.json` 은 2026-09-23 채팅 은퇴로 사라졌다)와 `.knowledge/`(백엔드
...
| `resolveScheduledCampaignStatus` · `syncCampaignStatusesBySchedule` | `src/lib/campaign-status-sync.ts` | 판매 캠페인 기간 도달/종료 자동 상태 전이 SSOT (종료 +1일 도달 `CLOSED`, 시작일 도달 `ACTIVE`, 체크리스트 연계 분리) |
...
| `findMissingProductOrderIds` · `collectProductOrderIds` | `src/lib/order-converter/naver-order-sync.ts` | 「네이버에 물어본 주문이 전부 돌아왔는가」(query 응답 완전성) 판정 SSOT(T-155). `queryOrderDetails` 의 미회신 경고와 `naver-settlement-sync.ts`(`syncPostCloseCancellations`)의 사후 취소 확정 게이트가 같은 판정을 각자 구현해 한쪽만 고치면 조용히 어긋날 수 있었다(이 저장소에 반복된 사고 유형 — 화면마다 같은 판정을 손으로 재구현하다 갈라진 것). ⛔ 개수로 재지 말 것 — 중복·잉여 행 하나가 빠진 id 를 가려 「온전함」으로 읽힌다(판정은 요청 id **집합**이 반환 집합에 전부 있는가). id 없는 행은 fail-closed 로 「모자람」에 든다. 두 소비처는 **단위가 다르다**(전자는 API 호출 100건 청크마다 즉시 판정, 후자는 300건씩 여러 청크를 캠페인 전체로 누적한 뒤 한 번 판정) — 그래서 완전 통합은 하지 않았고, `collectProductOrderIds` 는 그 차이를 흡수하려고 기존 Set 에 누적하는 두 번째 인자(`into`)를 받는다. 계약은 `naver-order-sync.test.ts` 의 「완전성 판정 SSOT, T-155」describe 블록(T-154 describe 블록과는 별개) · `settlement-post-close-lock.contract.test.ts` |
...
| `parseStoredJson` · `parseStoredJsonObject` | `src/lib/stored-json.ts` | 저장된 Json 컬럼 **읽기** SSOT(순수·client-safe). 🪤 **이 레포의 Json 은 프로바이더마다 모양이 다르다** — 리포지토리 3종(`actionProposalRepository`·`priceSheetRepository`·`priceMonitorSnapshotRepository`)이 "Postgres 는 객체, SQLite 는 문자열"로 이원화해 저장하고 **역직렬화는 리포지토리 쪽에만 있다.** raw Prisma 로 읽고 `value as T` 로 캐스팅하면 **Postgres 에선 통하고 SQLite 에서만 조용히 빈 값**이 된다 — 타입도 테스트도 못 잡는다(픽스처를 프로덕션 모양으로만 만들면 초록이다). 실사고 2건: ①기안 dedup 이 로컬에서 통째로 뚫려 중복 기안 생성 ②가격표 「반영 결과」가 성공한 반영을 "생성 0건"으로 표시. ⛔ Json 컬럼을 raw 로 읽었으면 캐스팅하지 말고 이 함수를 통과시킨다. 재발 방지는 `stored-json.contract.test.ts` 의 **필드 단위 소스 스캔** — ⚠️ 파일 단위로 느슨하게 바꾸지 말 것(파일 단위 판정은 위 ②를 놓쳤다, 양성 프로브로 확인). 행 통째로 넘기는 자리는 그 테스트의 `ROW_LEVEL_HANDLED` 에 이유와 함께 열거한다 |
...
| `assertAgentWorkerDbIdentity` · `AGENT_WORKER_DB_ROLE` | `src/lib/agent-worker/db-identity.ts` | Agent Worker 의 **DB 신원 확인** SSOT(P0 인접, T-117) — 워커가 기동 직후 DB 에 `current_user`/`session_user` 를 직접 물어 최소권한 role(`wag_agent_worker`)이 아니면 멈춘다. ⛔ **래퍼(`infra/selfhost/run-agent-worker.sh`)의 문자열 비교로 이 판정을 대신하지 말 것** — 그쪽은 "앱 `.env` 와 글자가 같은가"만 보므로 **같은 전체권한 계정이라도 접속 옵션 하나만 다르면 통과한다**(신원 확인이 아니라 철자 확인이다). 두 방어선은 보는 것이 달라 어느 한쪽으로 합칠 수 없다. ⚠️ **두 컬럼을 모두 본다**: `session_user` 는 로그인 role(= 틀린 `DATABASE_URL`), `current_user` 는 권한 평가 role(= 올바른 로그인 위의 `SET ROLE` 상승) — 하나만 보면 나머지 절반이 무방비다. 결과가 읽히지 않으면(행 수 이상·비문자열) **통과가 아니라 실패**이고, 드라이버 오류 메시지는 접속 대상을 담을 수 있어 다시 던지지 않는다(`cause` 로만 남긴다 — 로그 파일 P0). sqlite 레인은 role 개념이 없어 건너뛴다. 계약은 `db-identity.test.ts`(행위) + `agent-worker-boundary.contract.test.ts`(배선 — 호출이 `loop.start()` 보다 앞서고 `await` 되는지: 뒤에서 돌면 이미 job 을 집은 뒤다) |
...
| `decideSettlementQueryPlan` · `loadSettlementQueryPlan` · `isProductOrderLedgerRow` | `src/lib/order-converter/settlement-pending-dates.ts` | 정산 조회의 **대상 날짜 선정 SSOT**(P7) — 부를 날짜를 **달력이 아니라 「정산 대기 주문」에서** 뽑는다. 바로 위 `fetchPendingOrderWindow` 가 발주 조회에서 이미 쓰던 「증거가 있을 때만 조회」 원칙을 정산에 옮긴 것이다(종전 `runSettlementSync` 는 정산완료일 3일 + 결제일 21일을 데이터 유무와 무관하게 매일 훑었다). 결제일 축 경로는 셋이고 **전부 수렴한다**: 정산완료 행이 없는 주문의 결제일(10일 상한) · 주문이 있었던 최근 날짜(고정 횟수 재확인) · 취소·반품인데 원거래는 정산됐고 차감이 없는 주문의 결제일(60일 — ⚠️ **가속기일 뿐 보장이 아니다**: 취소 사실을 읽는 스냅샷 `claimSource` 가 평평한 모양의 클레임을 안 담고, 주문 동기화가 결제 30일 지난 주문의 변경을 버린다). 여기에 **정산완료일 축 안전망**(최근 끝난 2일, `SETTLEMENT_COMPLETE_DATE_LOOKBACK_DAYS`)이 매 회차 붙는다 — 취소 감지가 놓치는 차감을 받는 유일한 경로다. 한 회차 상한을 두되 **재확인 몫을 먼저 예약**하고 잘린 수는 `truncatedDates` 로 신고한다. 계획은 **DB 만 읽는다**(네이버 0회) — 그래서 크론의 `?dryRun=1` 이 공짜 검증이 되고, 그 분기는 `withSystemTaskStatus` **밖**에 있어 크론 상태를 쓰지 않는다. ⛔ **되돌리지 말 것 셋**(전부 교차 검증이 잡은 실결함): ①「받았는가」를 **스냅샷 주문 수 vs 원장 수** 대조로 판정 — 두 수는 술어가 달라(스냅샷은 결제일이 없으면 주문일로 폴백 귀속) `order-fetch-window.findChunkIntegrityIssues` 가 **차단 근거 금지**를 명시한 바로 그 대조이고 실측에서 양방향으로 어긋났다(관측 전용 `ledgerShortDates` 로만 남아 있다) ②상한을 `allDates.slice(0, MAX)` 로 — 차감이 영영 안 오는 옛 클레임이 매 회차 상한을 독점해 **오늘 주문일이 재확인 창 밖으로 밀려 영영 조회되지 않는다** ③dry-run 을 래퍼 안으로 — 실제 동기화 없이 SUCCESS 가 기록돼 레이더가 거짓 초록이 된다 ④「0콜을 위해」 정산완료일 안전망을 빼기 — 2단계 첫 커밋이 그렇게 했다가 반품 차감이 영영 조회되지 않는(금액 과대) 블로커가 두 리뷰 레인에서 독립적으로 잡혔다. `isProductOrderLedgerRow` 는 정산 원장을 **주문 단위로** 다루는 모든 곳의 공용 술어다(`recomputeClosedCampaignSettlements` 와 공유 — 종전엔 인라인 사본이 둘이었다). 배송비·리뷰적립 원장을 안 거르면 배송비 정산만으로 상품 결제까지 「끝났다」로 읽힌다. **정산 크론의 기본 경로다**(2단계) — 크론은 `runPlannedSettlementSync`(`naver-settlement-sync.ts`)로 이 계획만 조회하고(결제일 축은 `settleDecisionType` 없이 날짜당 1회 — 정산·미정산·차감이 한 응답에 온다), 고정 달력 `runSettlementSync` 는 `?settledDays=`/`?unsettledDays=` 를 명시로 줄 때만 도는 **백필 전용**이다(재확인 창보다 긴 크론 중단 구간의 유일한 복구 경로 — 지우지 말 것). 날짜별 실패는 격리되고 `failedDates` 가 있으면 크론이 실패로 선언된다. 설계 정본 `docs/private/specs/2026-09-10-settlement-query-redesign.md`, 계약은 `settlement-pending-dates.test.ts`(굶김·경계·무계수 배제 전부 단언) · `settlement-planned-sync.test.ts`(날짜당 1회 · 필터 없음 · 안전망 · 실패 격리) · `naver-settlement-sync/route.test.ts`(기본=계획 · 백필은 명시 요청만 · 실패 선언 · dry-run 무쓰기) |
| `decidePostCloseCheck` · `isPostCloseTerminalOrder` · `nextAllTerminalMarker` · `postCloseCandidateEndDateFloor` | `src/lib/order-converter/post-close-check-window.ts` | 마감 캠페인 **사후 취소·교환 확인 기간 SSOT**(P7) — 전 주문 종결 최초 관측 시각(`OrderCampaign.cachedPostCloseAllTerminalAt`) +10일(KST 날짜)에 멈추고(판매 종료일과 무관), 종결을 한 번도 못 본 캠페인만 판매 종료 +15일에서 멈춘다(오너 확정·승인 2026-09-11, 종전 90일 창 대체). 클레임 종결은 완료·거부 상태 목록으로 본다 — ⛔ claim-derive `isCompleted`(부분 문자열)는 `COLLECT_DONE` 을 완료로 읽어 쓰지 않는다. 종결 미관측 상한은 수동 레버 `includeLocked` 도 넘지 않는다. 소비처 `syncPostCloseCancellations`(`naver-settlement-sync.ts`). 설계 `docs/private/plans/2026-09-11-post-close-cancel-window.md`, 계약 `post-close-check-window.test.ts` · `settlement-post-close-lock.contract.test.ts` |
