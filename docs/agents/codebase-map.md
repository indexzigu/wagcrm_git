...
| `isSupplementProduct` · `SUPPLEMENT_PRODUCT_CLASS` | `src/lib/order-converter/product-class.ts` | 네이버 `productClass` 로 **추가구성상품(추가옵션)인가**를 가르는 판정 SSOT(P7). 이 술어는 주문 귀속의 분기점이라 — 참이면 1차 매칭에서 빼고 2차 패스(동일 `productId` 메인 품목이 이 캠페인에 귀속됐는지)로 미룬다 — 집계·발주 표면마다 필요한데, 종전에는 **7곳에 손으로 복사**돼 있었다(캠페인 핸들러 · 발주 실행 2경로 · 모바일 펄스 · 마감 캐시 · 캠페인 주문 · 미발주 목록). 사본이 갈려도 **타입도 테스트도 잡지 못하고 실패가 조용하다** — 라인이 메인으로 오분류되면 매칭에 실패해 집계에서 사라질 뿐 오류가 나지 않는다(`INVALID_ORDER_STATUSES` 가 오타 하나로 전 지표를 부풀린 것과 같은 부류, P7 Valid-Order Enum Discipline). ⚠️ **2차 귀속 규칙 자체는 표면마다 다르므로 이 모듈이 갖지 않는다** — 분류만 답한다. 비교 전에 `toNfc` 를 거치는데, `productClass` 는 네이버 응답이 스냅샷에 **그대로** 저장되는 값이라(저장 경로에 유니코드 정규화가 없다) 자모 분리형이 섞이면 정확 일치가 조용히 빗나가기 때문이다. ⛔ `normalizeForCompare` 로 갈아타지 말 것 — 공백·대소문자까지 뭉개는데 이 값은 네이버가 정한 **분류 enum** 이라 정확 일치 자리다. 계약은 `product-class.contract.test.ts`(AST 소스 스캔 — 🪤 소비처 단언은 이름 등장이 아니라 **호출 노드**를 센다: 초판은 `includes()` 로 재서, 호출부를 리터럴 비교로 되돌려도 `import` 줄만으로 초록이었다) · 행위는 `product-class.test.ts` |
