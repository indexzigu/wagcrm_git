...
## ⚠️ PR 번호는 레포를 가려서 읽는다 (실사고 2026-07-31)

이 레포는 2026-07-16 구 레포 `indexzigu/wag-crm` 에서 이관됐고 **두 레포의 PR 번호는
독립적으로 매겨졌다.** 그래서 같은 번호가 전혀 다른 작업을 가리킨다 — 실측된 충돌:

| 번호 | 구 레포 (`wag-crm`) | 현 레포 (`wagcrm`) |
| --- | --- | --- |
| #187 | `6e0f5b4` docs(agents) 색 사용처 | `5725dd2` fix(claims) 채택분 출처 배선 |
| #188 | `4e67fe6` fix(auth) 로그인 safe-area | `e9ca4bb` fix(claims) 모델명 출처 |

**증상:** 보드·핸드오프의 맨 `#188` 을 오늘 번호로 읽어 "이미 착지한 작업"에 재착수
지시가 나가거나, 반대로 진행 중인 작업을 완료로 오판한다. 2026-07-31 에 실제로 보드
한 줄이 두 작업을 동시에 가리키는 상태로 발견됐다.
...
으로 확인한다 — `git show origin/release:<파일> | grep <핵심 문자열>`. 해시로 찾다가
...
- **항상 신뢰(살아있는 정본):** `PROJECT_MASTER.md`(작업 보드)와
  ...
  `docs/runbooks/*.md`(**오너 대면 운영 절차** — 계정 개설 등 사람이 손으로 밟는 단계.
