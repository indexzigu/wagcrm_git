...
| `resolveImapHost` · `resolveSmtpConfig` · `orderMailboxesForScan` · `isOwnSenderAddress` · `toNfc` | `src/lib/mail-config.ts` | 메일 서버 **좌표·편지함 순회 정책** SSOT. 소비처 셋(`tax-invoice-mail/mail-scan.ts` 수취 계산서 스캔 · `order-converter/api/fetch-emails` 송장 회신 · `order-converter/api/send-email` 발주 발송)이 **같은 계정 자격증명**(`SMTP_USER`/`SMTP_PASS`)을 공유하는데 호스트만 각자 리터럴로 박혀 있었다 — 계정을 옮기면 고칠 곳이 셋인데 **한 곳을 빠뜨려도 타입도 테스트도 못 잡는다**(그 기능만 옛 서버에 새 자격증명으로 붙어 인증 실패). 2026-09-01 다음메일→구글 전환의 계기다. ⛔ 소비처에 호스트를 다시 적지 말 것 — `mail-config.contract.test.ts` 가 소스 스캔으로 막는다(🪤 주석을 걷어내고 스캔한다: 이 트랙의 문서가 옛·새 서버 이름을 **설명하려고** 인용한다. 양성 프로브로 SSOT 자신에는 매치가 있음을 함께 단언해 스캐너 고장이 초록으로 보이는 것을 막는다). 🔑 **서버는 계정 주소에서 파생한다 — 상수로 되돌리지 말 것**: 셀프호스트는 `main` 을 pull 해 배포하므로, 구글을 무조건 기본으로 두면 오너가 `.env` 를 바꾸기 **전에** 배포가 도는 순간 옛 자격증명이 구글로 가서 수신 2경로·발신 1경로가 **동시에** 죽는다(env 점검기는 「비었는가」만 보므로 원리적으로 못 잡는다 — 교차 검증 P1). 옛 사업자 폴백의 존속은 계약 테스트가 **있어야 통과**하는 방향으로 고정한다. ⚠️ **구글은 일반 비밀번호를 안 받는다**(2025-03-14 차단) — `SMTP_PASS` 는 앱 비밀번호이고, OAuth 는 메일함 전체 접근이 제한 범위 스코프라 CASA 감사가 붙어 기각했다. ⚠️ **발신인 치환이 침묵형이다**: `SMTP_FROM_EMAIL` 의 주소를 구글 계정에 **발신 주소로 등록·인증**하지 않으면 오류가 아니라 **로그인 계정 주소로 조용히 바뀌어** 나간다(로그는 성공, 브랜드사 화면에서만 드러난다) — `isOwnSenderAddress` 가 로그인 계정 주소도 함께 보는 이유이고(안 보면 우리가 보낸 발주서 원본이 회신으로 오인된다), env 표에서 `SMTP_FROM_EMAIL` 이 `degrades` 인 이유다. 🪤 **편지함 제외를 이름 목록으로 다시 만들지 말 것**: 종전 인라인 목록은 다음메일의 **띄어쓴** 한국어 이름(`지운 편지함`)만 알아서 구글의 `휴지통`·`보낸편지함`·`전체보관함`·`중요편지함`이 하나도 안 걸렸다(전체보관함·중요편지함은 다른 폴더의 사본이라 같은 메일을 두 번 훑는다). 판정은 ①IMAP 특수용도 속성 ②공백을 걷어낸 이름 폴백 두 겹이다. ℹ️ **`전체보관함`은 제외가 아니라 맨 뒤로 미룬다** — 오너가 회신을 읽고 보관하면 그 메일은 거기에만 남으므로 빼면 영영 「회신 없음」이 되고, 호출부가 첫 발견에서 순회를 멈추므로 앞에서 찾으면 비용이 0 이다. 🔴 **구글에서만 터지는 함정 2종(2026-09-02 배포 전 실접속 점검에서 발견 — 다음메일에서는 둘 다 안 드러난다)**: ①**SNI** — `node-imap` 이 평문 소켓을 먼저 열고 `tls.connect({host, socket})` 에 넘기는데, 소켓을 주면 Node 가 `host` 에서 SNI 를 유추하지 않는다. 없으면 구글이 다른 인증서를 내줘 `self-signed certificate` 로 끊기고 **수신 2경로가 동시에** 죽는다(대조군 실측: `imap.daum.net` 은 SNI 유무 무관). `resolveImapConfig` 의 `tlsOptions.servername` 이 그것이고 ⛔ 지우지 말 것 — 계약이 **`servername: host` 대입**을 본다(🪤 `tlsOptions: { servername` 로 재면 **타입 선언에도 걸려** 대입을 지워도 초록이다, 변이 테스트 실측). ②**자모 분리(NFD)** — 구글은 한글 편지함 이름을 NFD 로 돌려준다(`세금계산서` 가 코드포인트 12개 vs 조합형 5개). 정확 일치도 부분 일치도 실패해 `pickTaxInvoiceBox` 가 조용히 `INBOX` 로 폴백하고, 라벨 규칙에 「받은편지함 건너뛰기」가 걸려 있으면 **스캔이 0건**이 되는데 화면에는 「메일 미발견」으로 보여 미수취와 구분되지 않는다. 정규화 원시함수는 `toNfc` 하나이고 **비교만 정규화하고 반환은 서버 원문**을 유지한다(`openBox` 는 서버 어휘로 열어야 한다). ⚠️ **편지함 이름만의 문제가 아니다** — 제목·첨부 파일명도 같은 축이고 그쪽은 형태를 **보낸 사람**이 정해서(맥에서 온 첨부는 파일명 NFD 가 상시) 실측으로 미리 못 거른다. 서버 문자열을 우리 한국어 상수와 맞대는 자리는 전부 `toNfc` 를 거친다. 오너 절차는 `docs/runbooks/gmail-mail-cutover.md` |
