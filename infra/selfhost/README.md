...
워커를 `kickstart` 하고, PID 교체와 3초 뒤 PID 유지까지 확인한다(프로덕션 레인 한정).
위 명령은 배포와 무관하게 손으로 재시작할 때만 쓴다.
⛔ **`bootout` 해 둔 채로는 배포가 통과하지 않는다.** plist 가 설치돼 있는데 launchd 에
올라가 있지 않으면 `deploy.sh` 는 건너뛰지 않고 **중단한다** — "배포는 성공했는데 워커는
아예 돌지 않는다"를 조용히 남기지 않기 위해서다. 그래서 **워커를 내려두고 포그라운드로
직접 돌리는 디버깅 워크플로 중에는 배포가 실패한다.** 배포하려면 먼저 수동 프로세스를
끄고 `launchctl bootstrap` 으로 되돌려 놓을 것(둘을 동시에 띄우면 워커가 이중으로 돈다).
워커를 아예 운영하지 않는 기계는 plist 가 없으므로 그냥 건너뛴다.
