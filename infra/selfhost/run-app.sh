...
#   · 그 한 번을 포함해, deploy.sh 안전장치 ⑧ 이 **헬스체크 뒤에** 앱 프로세스의 cwd 를
#     읽어 릴리스 트리를 서빙 중인지 확인하고, 아니면 배포를 실패시킨다(마커 미갱신 →
#     다음 실행이 재시도). 🪤 `ps` 로는 판정할 수 없다 — Next 가 프로세스 이름을
#     `next-server (v16.2.4)` 로 갈아치워 실행 경로가 남지 않는다(2026-08-29 실측).
